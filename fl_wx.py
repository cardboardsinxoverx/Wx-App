...
# -*- coding: utf-8 -*-
"""
Standalone Florida (FL) Weather Map Generator.
...
NEXRAD radar image to generate a detailed weather map for the state of Florida.

The final map is saved as 'fl_detailed_weather.png'.
"""
...
# Pre-generate icons on script load
...
# --- Configuration (FLORIDA) ---
MAP_EXTENT = [-88.5, -79.5, 23.8, 31.5] # [lon_min, lon_max, lat_min, lat_max]
CITIES = {
    "Miami": (25.7617, -80.1918, "KMIA"),
    "Tampa": (27.9755, -82.5330, "KTPA"),
    "Orlando": (28.4294, -81.3089, "KMCO"),
    "Jacksonville": (30.3322, -81.6557, "KJAX"),
    "Fort Lauderdale": (26.0726, -80.1528, "KFLL"),
    "Tallahassee": (30.4383, -84.3514, "KTLH"),
    "Pensacola": (30.4735, -87.1871, "KPNS"),
    "Gainesville": (29.6911, -82.2711, "KGNV"),
    "Sarasota": (27.3956, -82.5548, "KSRQ"),
    "Daytona Beach": (29.1792, -81.0583, "KDAB"),
    "Key West": (24.5551, -81.7800, "KEYW"),
    "Panama City": (30.2079, -85.7974, "KECP"),
    "Fort Myers": (26.4822, -81.7696, "KRSW"),
    "West Palm Beach": (26.6832, -80.0955, "KPBI"),
    "Melbourne": (28.1028, -80.6453, "KMLB"),
    "St. Petersburg": (27.7709, -82.6266, "KPIE"),
    "Naples": (26.1527, -81.7753, "KAPF"),
}
CITY_OFFSETS = {
    # SE Coast Cluster
    "Miami": (0.15, -0.15),
    "Fort Lauderdale": (0.3, 0.1),
    "West Palm Beach": (0.3, 0.2),
    
    # Tampa Bay Cluster
    "Tampa": (0.15, 0.1),
    "St. Petersburg": (-0.45, 0.1),  # MOVED: Pushed further LEFT
    "Sarasota": (0.2, -0.2),
...
SILENT_STATIONS = {
    # In-state fill-ins
    "Apalachicola": (29.728, -85.029, "KAAF"),
    "Ocala": (29.171, -82.224, "KOCF"),
    "Vero Beach": (27.656, -80.418, "KVRB"),
    "Marathon": (24.726, -81.051, "KMTH"),
    "Cross City": (29.626, -83.104, "KCTY"),
    "St. Augustine": (29.959, -81.340, "KSGJ"),
    "Lakeland": (28.089, -81.950, "KLAL"),

    # Border stations for "pinning" the edges
    "Mobile (AL)": (30.6869, -88.2428, "KMOB"),
    "Dothan (AL)": (31.3214, -85.4494, "KDHN"),
    "Valdosta (GA)": (30.8327, -83.2785, "KVLD"),
    "Waycross (GA)": (31.2136, -82.3549, "KAYS"),
    "Brunswick (GA)": (31.1518, -81.3911, "KBQK"),
}
ALL_STATIONS_FL = {**CITIES, **SILENT_STATIONS}
# ---
FIG_BG_COLOR = '#b0c4de'
LAND_COLOR = '#f5f5dc'
OCEAN_COLOR = '#b0e0e6'
...
def get_nws_alerts(state='FL'):
    """Fetches active NWS alerts for a given state (e.g., 'FL')."""
...
def generate_florida_weather_map():
    """Main function to generate the Florida weather map."""
    ...
    metar_cache_file = "metar_fl_cache.json"
    ...
    if metar_results is None:
        ...
        metar_results = asyncio.run(fetch_all_metars_async(ALL_STATIONS_FL))
    ...
    alert_cache_file = "alerts_fl_cache.json"
    alerts = load_json_cache(alert_cache_file, CACHE_DURATION_SECONDS)
    
    if alerts is None:
        logging.info("Fetching NWS alerts for Florida...")
        alerts = get_nws_alerts('FL') # This now calls the *working* function
    ...
    for city, temp in all_temperatures.items():
        lat, lon, _ = ALL_STATIONS_FL[city]
    ...
    fig.suptitle(f"Florida - Detailed Weather Map\nGenerated: {current_time}",
    ...
    clip_patch = None
    try:
        logging.info("Creating state clipping path for Florida...")
        ...
        if not state_geometries:
            raise ValueError("Could not find Florida in shpreader.")
        
        # Combine geometries
        combined_geom = unary_union(state_geometries)
        if not combined_geom:
            raise ValueError("Unary union of Florida geometry failed.")
            
        # Convert the Shapely geometry to a Matplotlib Path
        paths = geos_to_path(combined_geom)
        if not paths:
            raise ValueError("geos_to_path returned no paths for Florida.")
        ...
        if clip_patch:
            ...
            raise ValueError("PathPatch creation failed for Florida.")
    except Exception as e:
        logging.error(f"Failed to create clip patch for Florida. Error: {e}")
        clip_patch = None
    ...
    for city, temp in all_temperatures.items():
        base_lat, base_lon, icao = ALL_STATIONS_FL[city]
    ...
    legend_handles = [
        ...
        mlines.Line2D([], [], color='#4169e1', marker='_', linestyle='None', markersize=12, mew=3, label='Altimeter (inHg)'),
    ...
    wwa_text = "### 🔔 Active Weather Alerts in Florida:\n" 
    if alerts:
        event_groups = {}
        for alert in alerts:
            evt = alert['event']
        ...
        wwa_text = "✅ No active weather alerts for Florida at this time."
...
# --- *** NEW DUAL-SAVE LOGIC *** ---
    ...
    dynamic_filename = f"fl_detailed_weather_{timestamp_str}.png"
    static_filename = "fl_detailed_weather.png" # For the GUI preview
    ...
    # --- Manually set the bottom margin for plot window ---
...
if __name__ == "__main__":
    ...
    try:
        generate_florida_weather_map() # <-- Now a synchronous call

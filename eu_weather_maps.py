...
def plot_background(ax):
    """Set up map background for Europe."""
...
async def generate_map(ds, run_date, level, variable, cmap, title, cb_label, levels=None):
    ...
    try:
        ...
        output_path = os.path.join(OUTPUT_DIR, f"eu_{variable}_{level//100}_{run_date.strftime('%Y%m%d_%H%MZ')}.png")
...
async def generate_mslp_temp_map():
    ...
    try:
        ds, run_date = await fetcher.fetch_gfs_surface(
            run_date, {"north": 70, "south": 35, "east": 45, "west": -25}
        )
        if ds is None:
        ...
        ax.set_title(f"Europe: MSLP with Temperature Gradient (°C) and Features", fontsize=16)
        ...
        plt.subplots_adjust(left=0.01, right=0.99, top=0.90, bottom=0.05)
        output_path = os.path.join(OUTPUT_DIR, f"eu_mslp_temp_{run_date.strftime('%Y%m%d_%H%MZ')}.png")
        plt.savefig(output_path, format='png', bbox_inches='tight')
        plt.close(fig)
...
async def generate_thermal_wind_map(level_lower=85000, level_upper=50000):
    ...
    try:
        ds, run_date = await fetcher.fetch_gfs(
            ...
            {"north": 70, "south": 35, "east": 45, "west": -25}
        )
        if ds is None:
            return None
        time_dims = get_time_dimension(ds, run_date)
        ...
        ax.barbs(lon_2d[::5, ::5], lat_2d[::5, ::5], u_wind_knots[::5, ::5], v_wind_knots[::5, ::5],
                 transform=crs, length=6)
        ax.set_title(f"{int(level_upper/100)} hPa Heights, {int(level_lower/100)}-{int(level_upper/100)} hPa Thickness, and {int(level_upper/100)} hPa Wind (Europe)", fontsize=16)
        plot_background(ax)
        plt.subplots_adjust(left=0.01, right=0.99, top=0.90, bottom=0.05)
        output_path = os.path.join(OUTPUT_DIR, f"eu_thermal_wind_{run_date.strftime('%Y%m%d_%H%MZ')}.png")
        plt.savefig(output_path, format='png', bbox_inches='tight')
        plt.close(fig)
...
async def eu_wind300():
    """Generate 300 hPa wind map."""
    print('Generating 300 hPa wind map for Europe...')
    fetcher = DataFetcher()
    now = datetime.utcnow()
    run_hours = [0, 6, 12, 18]
    hours_since_midnight = now.hour + now.minute / 60
    ...
    try:
        ...
        output_path = await generate_map(
            ...
            '300-hPa Wind Speeds and Heights (Europe)', 'Wind Speed (knots)'
        )
        if output_path:
        ...
        logger.error(f"Error in eu_wind300: {e}")
        print('Failed to generate the 300 hPa wind map.')

async def eu_wind500():
    """Generate 500 hPa wind map."""
    print('Generating 500 hPa wind map for Europe...')
    fetcher = DataFetcher()
    now = datetime.utcnow()
    run_hours = [0, 6, 12, 18]
    hours_since_midnight = now.hour + now.minute / 60
    ...
    try:
        ...
        output_path = await generate_map(
            ...
            '500-hPa Wind Speeds and Heights (Europe)', 'Wind Speed (knots)'
        )
        if output_path:
            print(f"Map saved to {output_path}")
        else:
        ...
        logger.error(f"Error in eu_wind500: {e}")
        print('Failed to generate the 500 hPa wind map.')

async def eu_vort500():
    """Generate 500 hPa relative vorticity map."""
    print('Generating 500 hPa relative vorticity map for Europe...')
    fetcher = DataFetcher()
    now = datetime.utcnow()
    run_hours = [0, 6, 12, 18]
    hours_since_midnight = now.hour + now.minute / 60
    ...
    try:
        ...
        output_path = await generate_map(
            ...
            '500-hPa Relative Vorticity and Heights (Europe)',
        ...
        logger.error(f"Error in eu_vort500: {e}")
        print('Failed to generate the 500 hPa relative vorticity map.')

async def eu_fronto700():
    """Generate 700 hPa frontogenesis map."""
    print('Generating 700 hPa frontogenesis map for Europe...')
    fetcher = DataFetcher()
    now = datetime.utcnow()
    run_hours = [0, 6, 12, 18]
    hours_since_midnight = now.hour + now.minute / 60
    ...
    try:
        ...
        output_path = await generate_map(
            ...
            '700-hPa Frontogenesis and Heights (Europe)',
        ...
        logger.error(f"Error in eu_fronto700: {e}")
        print('Failed to generate the 700 hPa frontogenesis map.')

async def eu_rh700():
    """Generate 700 hPa relative humidity map."""
    print('Generating 700 hPa relative humidity map for Europe...')
    fetcher = DataFetcher()
    now = datetime.utcnow()
    run_hours = [0, 6, 12, 18]
    hours_since_midnight = now.hour + now.minute / 60
    ...
    try:
        ...
        output_path = await generate_map(
            ...
            '700-hPa Relative Humidity and Heights (Europe)', 'Relative Humidity (%)'
        )
        if output_path:
            print(f"Map saved to {output_path}")
        else:
        ...
        logger.error(f"Error in eu_rh700: {e}")
        print('Failed to generate the 700 hPa relative humidity map.')

async def eu_wind850():
    """Generate 850 hPa wind map."""
    print('Generating 850 hPa wind map for Europe...')
    fetcher = DataFetcher()
    now = datetime.utcnow()
    run_hours = [0, 6, 12, 18]
    hours_since_midnight = now.hour + now.minute / 60
    ...
    try:
        ...
        output_path = await generate_map(
            ...
            '850-hPa Wind Speeds and Heights (Europe)', 'Wind Speed (knots)'
        )
        if output_path:
            print(f"Map saved to {output_path}")
        else:
        ...
        logger.error(f"Error in eu_wind850: {e}")
        print('Failed to generate the 850 hPa wind map.')

async def eu_dew850():
    """Generate 850 hPa dewpoint map."""
    print('Generating 850 hPa dewpoint map for Europe...')
    fetcher = DataFetcher()
    now = datetime.utcnow()
    run_hours = [0, 6, 12, 18]
    hours_since_midnight = now.hour + now.minute / 60
    ...
    try:
        ...
        output_path = await generate_map(
            ...
            '850-hPa Dewpoint (°C) (Europe)', 'Dewpoint (°C)',
        ...
        logger.error(f"Error in eu_dew850: {e}")
        print('Failed to generate the 850 hPa dewpoint map.')

async def eu_mAdv850():
    """Generate 850 hPa moisture advection map."""
    print('Generating 850 hPa moisture advection map for Europe...')
    fetcher = DataFetcher()
    now = datetime.utcnow()
    run_hours = [0, 6, 12, 18]
    hours_since_midnight = now.hour + now.minute / 60
    ...
    try:
        ...
        output_path = await generate_map(
            ...
            '850-hPa Moisture Advection (Europe)', 'Moisture Advection (%/hour)'
        )
        if output_path:
            print(f"Map saved to {output_path}")
        else:
        ...
        logger.error(f"Error in eu_mAdv850: {e}")
        print('Failed to generate the 850 hPa moisture advection map.')

async def eu_tAdv850():
    """Generate 850 hPa temperature advection map."""
    print('Generating 850 hPa temperature advection map for Europe...')
    fetcher = DataFetcher()
    now = datetime.utcnow()
    run_hours = [0, 6, 12, 18]
    hours_since_midnight = now.hour + now.minute / 60
    ...
    try:
        ...
        output_path = await generate_map(
            ...
            '850-hPa Temperature Advection (Europe)', 'Temperature Advection (K/hour)',
            levels=np.linspace(-20, 20, 41)
        )
        if output_path:
            print(f"Map saved to {output_path}")
        ...
        logger.error(f"Error in eu_tAdv850: {e}")
        print('Failed to generate the 850 hPa temperature advection map.')

async def eu_mslp_temp():
    """Generate MSLP with temperature gradient map."""
    print('Generating MSLP with temperature gradient map for Europe...')
...
async def eu_divcon300():
    """Generate 300 hPa divergence map."""
    print('Generating 300 hPa divergence map for Europe...')
    fetcher = DataFetcher()
    now = datetime.utcnow()
    run_hours = [0, 6, 12, 18]
    hours_since_midnight = now.hour + now.minute / 60
    ...
    try:
        ...
        output_path = await generate_map(
            ...
            '300-hPa Divergence/Convergence (Europe)', r'Divergence ($10^{-5}$ s$^{-1}$)'
        )
        if output_path:
            print(f"Map saved to {output_path}")
        else:
        ...
        logger.error(f"Error in eu_divcon300: {e}")
        print('Failed to generate the 300 hPa divergence/convergence map.')

async def eu_thermal_wind():
    """Generate thermal wind map."""
    print('Generating thermal wind map for Europe...')

...
from weather_maps import (
    wind300, wind500, vort500, rh700, fronto700, wind850, dew850, mAdv850, tAdv850, mslp_temp, divcon300
)
from eu_weather_maps import (
    eu_wind300, eu_wind500, eu_vort500, eu_rh700, eu_wind850, eu_dew850, eu_mAdv850, eu_tAdv850, eu_mslp_temp, eu_divcon300
)
try:
    ...
    logging.error(f"Failed to import local modules: {e}. Ensure utils.py, weather_calculations.py, and RTMA_Graphics.py are in the same directory.")
...
def metar_command(icao, hoursback=0):
    ...
    try:
        ...
        logging.info(f"Requested METAR for {icao} (hoursback={hoursback})")
    except Exception as e:
        print(f"Error fetching METAR: {e}")

# --- TAF Command ---
def get_taf(icao, hoursback=0):
    try:
        taf_url = f'https://aviationweather.gov/api/data/taf?ids={icao}&format=json&hours={hoursback}'
...
def taf_command(icao, hoursback=0):
    icao = icao.upper()
    try:
        raw_tafs = get_taf(icao, hoursback)
        if hoursback > 0:
            print(f"TAFs for {icao} (Last {hoursback} Hours):")
            for i, raw_taf in enumerate(raw_tafs):
                print(f"Forecast {i+1}:\n{raw_taf}\n")
        else:
            print(f"TAF for {icao}:\n{raw_tafs[0]}")
        logging.info(f"Requested TAF for {icao} (hoursback={hoursback})")
    except Exception as e:
...
async def meteogram_command(icao, hoursback):
    try:
        ...
        logging.error(f"Error in meteogram_command for {icao}: {e}")
...
async def windrose_command(longitude, latitude, start_date, end_date):
    try:
        ...
        logging.error(f"Error in windrose_command: {e}")

# --- Active Storms ATL Command ---
    ...
    try:
        async with session.get(url, headers=headers, timeout=aiohttp.ClientTimeout(total=30)) as response:
            if response.status == 403:
                logging.error(f"403 Forbidden on {url}, retrying with delay")
            ...
            if not tables:
                logging.warning("No tables found, trying alternative selectors")
                ...
                for row in table.find_all('tr')[1:]:
                    ...
                    if len(cols) > 2:
                        ...
                        if lat_text and lon_text:
                            ...
                            vmax = float(cols[4].text.strip()) if len(cols) > 4 and cols[4].text.strip() else 'N/A'
                            mslp = float(cols[5].text.strip()) if len(cols) > 5 and cols[5].text.strip() else 'N/A'
            ...
            for text in disturbance_text:
                ...
                if invest_match:
                    ...
                    if lat_lon_match:
                        ...
                        storms.append({"id": invest_id, "name": "Potential Development", "lat": lat, "lon": lon, "vmax": 'N/A', "mslp": 'N/A'})
            logging.info(f"JTWC storms and invests retrieved: {storms}")
            return storms
    except Exception as e:
        logging.error(f"Failed to fetch JTWC storms: {e}, URL: {url}")
        return []
    ...
    try:
        async with session.get(url, headers=headers, timeout=aiohttp.ClientTimeout(total=30)) as response:
            ...
            for invest_id, lat_text, lon_text in invest_matches:
                ...
                invests.append({"id": invest_id, "lat": lat, "lon": lon, "name": "Potential Development", "vmax": 'N/A', "mslp": 'N/A'})
            logging.info(f"NHC invests retrieved: {invests}")
            return invests
    except Exception as e:
        logging.error(f"Failed to fetch NHC invests: {e}, URL: {url}")
        return []
...
async def active_storms_atl_command():
    ...
    async with aiohttp.ClientSession() as session:
        try:
            logging.info("Fetching active storms from NHC")
            active_storms = realtime_obj.list_active_storms(basin='north_atlantic')
            invests = await fetch_nhc_invests(session)
            all_storms = [{"id": s, "name": realtime_obj.get_storm(s).name, "lat": realtime_obj.get_storm(s).lat[-1] if isinstance(realtime_obj.get_storm(s).lat, list) else realtime_obj.get_storm(s).lat,
                          "lon": realtime_obj.get_storm(s).lon[-1] if isinstance(realtime_obj.get_storm(s).lon, list) else realtime_obj.get_storm(s).lon,
                          "vmax": realtime_obj.get_storm(s).vmax if realtime_obj.get_storm(s).vmax else 'N/A',
                          "mslp": realtime_obj.get_storm(s).mslp if realtime_obj.get_storm(s).mslp else 'N/A'} for s in active_storms if s.endswith('L')]
            all_storms.extend(invests)
            if not all_storms:
            ...
            logging.error(f"Error in active_storms_atl_command: {e}")

# --- Satellite Imagery Command ---
image_links = {
    'conus': {
...
async def sat_command(region, product_code):
    ...
    try:
        ...
        logging.error(f"Error in sat_command: {e}")
...
def radar_command(region='chase', overlay='base'):
    ...
    try:
        ...
        logging.error(f"Error in radar_command: {e}")
...
async def astro_command(location, time_str=None):
    ...
    try:
        ...
        logging.error(f"Error in astro_command: {e}")
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(alerts_url, headers={'User-Agent': 'WeatherApp/1.0'}) as response:
                ...
                logging.info(f"Requested alerts for {state_abbr}")
    except Exception as e:
        print(f"Error fetching alerts: {e}")
        logging.error(f"Error in alerts_command: {e}")
...
async def forecast_command(location):
    try:
        ...
        if '/' in location:
            try:
                lat, lon = map(float, location.split('/'))
                logging.info(f"Resolved {location} to coordinates: ({lat}, {lon})")
            except ValueError:
                print("Invalid lat/lon format. Use 'lat/lon' (e.g., '34.05/-118.25').")
                return
        elif location.upper() in location_data or location in location_data:
            lat, lon = location_data.get(location.upper(), location_data.get(location))
            logging.info(f"Resolved {location} to coordinates via local data: ({lat}, {lon})")
                ...
                async with session.get(geonames_url, headers={'User-Agent': 'WeatherApp/1.0'}) as response:
                    if response.status == 200:
                        ...
                        if data.get('postalCodes'):
                            ...
                            logging.info(f"Resolved ZIP {location} to coordinates via GeoNames: ({lat}, {lon})")
                        else:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(geocode_url, headers={'User-Agent': 'WeatherApp/1.0'}) as response:
                    if response.status == 200:
                        ...
                        logging.info(f"Resolved {location} to coordinates via NWS API: ({lat}, {lon})")
                    else:
                        print(f"Could not find location: {location}. Try a city name, ZIP code, ICAO code, or lat/lon.")
                        return
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(nws_zone_url, headers={'User-Agent': 'WeatherApp/1.0'}) as response:
                if response.status == 200:
                    ...
                    logging.info(f"Resolved coordinates ({lat}, {lon}) to NWS zone: {zone_id}")
                else:
        ...
        if issuance_match:
            try:
                ...
                logging.warning(f"Could not parse issuance time for zone {zone_id}: {e}")
                print("Could not verify forecast issuance time. Proceeding with available data.")
        else:
            logging.warning(f"Could not find issuance time in forecast for zone {zone_id}.")
            print("Could not verify forecast issuance time. Proceeding with available data.")
        print(f"\nWeather Forecast for {location.title()} (Zone: {zone_id}):")
        print("```\n" + forecast_text.strip() + "\n```")
        logging.info(f"Requested forecast for {location} (Zone: {zone_id})")
    except Exception as e:
        print(f"Error in forecast command: {str(e)}")
        logging.error(f"Error in forecast_command for {location}: {str(e)}")
...
def worldtimes_command():
    ...
    logging.info("Requested world times")

def print_help():
    print("""
Available Commands:
  metar <icao> [hoursback] - Fetch METAR data for an airport (e.g., 'metar KMGE 6')
  taf <icao> [hoursback] - Fetch TAF data for an airport (e.g., 'taf KMGE 6')
    ...
    while True:
        ...
        try:
            if cmd_name == "restart":
                ...
                for task in tasks:
                    task.cancel()
                # Restart the script using os.execv
                ...
                if len(command) < 2:
                    print("Usage: taf <icao> [hoursback]")
                    continue
                icao = command[1]
                hoursback = int(command[2]) if len(command) > 2 else 0
                taf_command(icao, hoursback)
                ...
                await meteogram_command(icao, hoursback)
                ...
                end_date = command[4]
                await windrose_command(longitude, latitude, start_date, end_date)
            elif cmd_name == "active_storms_atl":
                await active_storms_atl_command()
            elif cmd_name == "sat":
                if len(command) < 3:
                ...
                await astro_command(location, time_str)
            elif cmd_name == "alerts":
                state_abbr = command[1] if len(command) > 1 else None
                await alerts_command(state_abbr)
            elif cmd_name == "forecast":
                if len(command) < 2:
                    print("Usage: forecast <location>")
                    continue
                location = ' '.join(command[1:])
                await forecast_command(location)
            elif cmd_name == "worldtimes":
                worldtimes_command()
            elif cmd_name == "skewt":
                if len(command) == 3:
                    station_code, sounding_time = command[1:3]
                    await skewt(station_code, sounding_time=sounding_time)
                elif len(command) == 4:
                    station_code, model, forecast_hour = command[1:4]
                    await skewt(station_code, model=model, forecast_hour=int(forecast_hour))
                else:
            ...
            print(f"Error: {e}")
            logging.error(f"Error in command {cmd_name}: {e}")

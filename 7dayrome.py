...
# Change these to your actual current paths
LOGO_PATH = "/media/evan/Main/frostbyte/frostbyte_project/boxlogo4.png"
...
def get_sun_times(lat_str, lon_str, date_obj):
    try:
        loc = LocationInfo("Rome", "Georgia", "US/Eastern", float(LAT), float(LON)); s = sun(loc.observer, date=date_obj.date(), tzinfo=loc.timezone)
...
def get_current_metar():
    try:
        r = requests.get("https://api.weather.gov/stations/KRMG/observations/latest", headers={"User-Agent": "(rome_fcst)"}, timeout=12)
...
def get_extra_open_meteo_data():
    try:
        ...
        if pollens:
            max_p = max(pollens, key=pollens.get)
            val = pollens[max_p]
            
            # FIXED THRESHOLDS: Models often under-report 
            # We treat 1.5+ as "High" to better match Georgia reality
            if val > 1.5: 
                pollen_text = f"High: {max_p.replace('_pollen','').capitalize()}"
            elif val > 0.3: 
                pollen_text = f"Mod: {max_p.replace('_pollen','').capitalize()}"
        
        # GEORGIA SEASONAL OVERRIDE: 
        # If it's March or April, we know the yellow pine dust is active 
        # regardless of what the global model says.
        current_month = datetime.now().month
        if current_month in [3, 4] and "High" not in pollen_text:
            pollen_text = "EXTREME: Pine/Oak"
...
def get_hourly_forecast():
    try:
        r = requests.get("https://api.weather.gov/gridpoints/FFC/52,79/forecast/hourly", headers={"User-Agent": "(rome_fcst)"}, timeout=15)
        if r.status_code != 200: return {}
        hourly_map = {}
...
def get_weather_data():
    try:
        ...
        daily = {}
        # Get today's date for relative indexing
        now_date = datetime.now().date()
        
        for p in periods:
            # Parse the actual date from the API instead of relying on the 'name' string
            start_time = datetime.fromisoformat(p["startTime"].replace("Z", "+00:00"))
            p_date = start_time.date()
            is_day = p["isDaytime"]
            
            # Use the date object as the dictionary key
            if p_date not in daily:
                daily[p_date] = {"day": None, "night": None}
            ...
            data_packet = {
                ...
                "text": final_text, 
                "icon_img": icon, 
                "wind_spd": p["windSpeed"], 
                "wind_dir": p["windDirection"], 
                "pop": pop_val
            }
            
            if is_day:
                daily[p_date]["day"] = data_packet
            else:
                daily[p_date]["night"] = data_packet

        ordered_days = []
        na_icon = load_icon_file("na@2x.png")
        
        # Build the 7-day list based on dates
        for i in range(7):
            target_date = now_date + timedelta(days=i)
            day_record = daily.get(target_date, {"day": None, "night": None})
            
            # Handle missing data cases
            if day_record["day"] is None:
                day_record["day"] = {"temp": "--", "text": "FCST PENDING", "icon_img": na_icon, "wind_spd": "", "wind_dir": "", "pop": 0}
            if day_record["night"] is None:
                day_record["night"] = {"temp": "--", "text": "FCST PENDING", "icon_img": na_icon, "wind_spd": "", "wind_dir": "", "pop": 0}
            
            # Labeling logic
            if i == 0:
                day_label = "TODAY"
            else:
                day_label = target_date.strftime("%A").upper()[:3]
                
            try:
                astro_text = get_sun_times(LAT, LON, datetime.now() + timedelta(days=i))
            except:
                astro_text = "Sun Data N/A"
                
            ordered_days.append({
                "day_label": day_label, 
                "astro_text": astro_text, 
                "day_data": day_record["day"], 
                "night_data": day_record["night"]
            })
...
def draw_full_graphic():
    print("Fetching forecast & Extra Data...")
    curr, data = get_weather_data(); hourly_data = get_hourly_forecast(); extras = get_extra_open_meteo_data()
    if not data: return
    live_now = get_current_metar(); moon_phases = [get_accurate_moon_phase(datetime.now() + timedelta(days=i)) for i in range(7)]
    ...
    center_x = 0.28; title_img = create_gradient_text_image("ROME, GA", 26, 'green'); ax_header.add_artist(AnnotationBbox(OffsetImage(title_img, zoom=0.5), (center_x, 0.65), frameon=False))
    ...
    # --- HEAD WEATHER ICON (UPDATED FOR SPLIT LOGIC) ---
    now = datetime.now()
    try:
        loc = LocationInfo("Rome", "Georgia", "US/Eastern", float(LAT), float(LON))
        s = sun(loc.observer, date=now.date(), tzinfo=loc.timezone)
        is_day_now = s['sunrise'].replace(tzinfo=None) < now < s['sunset'].replace(tzinfo=None)
    except: 
        is_day_now = 6 <= now.hour < 20
        
    # Use the smart split-logic function instead of the single icon function
    head_icon, _ = get_icon_by_text(live_now['condition'], is_day=is_day_now)
    
    if head_icon is not None:
        # Dynamic zoom to keep it looking sharp in the glass panel
        target_height = 85 # Slightly larger for the header
        zoom_val = target_height / head_icon.shape[0]
        ax_glass.add_artist(AnnotationBbox(OffsetImage(head_icon, zoom=zoom_val), (0.48, 0.5), frameon=False, zorder=12))
    ...
    # Left Col X = 0.65, Right Col X = 0.82
    
    col1 = []
    # Removing Sky from here since it's under the temp
    ...
    w = 1.0 / 7
    phase_map = {
        "New Moon": "new", "Waxing Crescent": "waxing_crescent", 
        "First Quarter": "first_quarter", "Waxing Gibbous": "waxing_gibbous", 
        "Full Moon": "full", "Waning Gibbous": "waning_gibbous", 
        "Last Quarter": "third_quarter", "Waning Crescent": "waning_crescent"
    }
    ...
    for i, day in enumerate(data):
        # Create the panel and IMMEDIATELY turn off the axis/ticks to remove the 0.0-1.0 marks
        ax = fig.add_axes([i*w, 0.05, w, 0.78])
        ax.axis('off') 
        ...
        bg = TODAY_BG_COLOR if i == 0 else PANEL_BG_COLOR
        ax.add_patch(mpatches.Rectangle((0,0), 1, 1, color=bg, zorder=0))
        ...
        day_img = create_gradient_text_image(lbl, 16, 'green')
        ax.add_artist(AnnotationBbox(OffsetImage(day_img, zoom=0.5), (0.5, 0.97), frameon=False, zorder=10))
        
        astro_img = create_gradient_text_image(day['astro_text'], 9, 'white')
        ax.add_artist(AnnotationBbox(OffsetImage(astro_img, zoom=0.35), (0.5, 0.92), frameon=False, zorder=10))
        
        # Day Data (High Temp, Icon, POP, Description)
        d_dat = day['day_data']
        y_day_center = 0.75 
        ...
        if d_dat:
            h_img = create_gradient_text_image(f"H:{d_dat['temp']}°F", 20, 'hot')
            ax.add_artist(AnnotationBbox(OffsetImage(h_img, zoom=0.5), (0.5, 0.86), frameon=False, zorder=10))
        ...
        if n_dat:
            ...
            m_icon = create_moon_icon(phase_map.get(moon_phases[i], "new"))
            ax.add_artist(AnnotationBbox(OffsetImage(m_icon, zoom=0.12), (0.85, 0.58), frameon=False, zorder=10))
            
            desc_img_n = create_gradient_text_image("\n".join(textwrap.wrap((n_dat['text']).upper(), 16)[:2]), 11, 'white')
            ax.add_artist(AnnotationBbox(OffsetImage(desc_img_n, zoom=0.35), (0.5, 0.42), frameon=False, zorder=10))

        # Hourly and Precip Charts
        ...
        if check_date in hourly_data: 
            ...
            app_temps = hourly_data[check_date]["app_temp"]
            dews = [d for d in hourly_data[check_date]["dew"] if d is not None]
            if app_temps and dews:
                ax.text(0.35, 0.37, f"Feels: {max(app_temps)}°F", color='#2E64FE', fontsize=7, ha='center', fontweight='bold', path_effects=[pe.withStroke(linewidth=1, foreground='black')], zorder=10)
                ax.text(0.65, 0.37, f"Dew: {int(sum(dews)/len(dews))}°F", color='#00FF00', fontsize=7, ha='center', fontweight='bold', path_effects=[pe.withStroke(linewidth=1, foreground='black')], zorder=10)

        # UV Badge (Today Only)
        if i == 0 and extras.get('uv') and extras.get('uv') > 2:
        ...
        # DRAW SEPARATOR LINE: Draws a thin line between every panel except the first
        if i > 0: 
            ax.plot([0, 0], [0.05, 0.95], color='#444444', lw=1, zorder=2, transform=ax.transAxes)

    os.makedirs(OUTPUT_DIR, exist_ok=True)
    path = os.path.join(OUTPUT_DIR, f"rome_7day_SPLIT_{datetime.now():%Y-%m-%d_%H%M%S}.png")

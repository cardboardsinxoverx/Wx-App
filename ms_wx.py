...
# -*- coding: utf-8 -*-
"""
Standalone Mississippi (MS) Weather Map Generator.
...
NEXRAD radar image to generate a detailed weather map for the state of Mississippi.

The final map is saved as 'ms_detailed_weather.png'.
"""
...
# --- Configuration (MISSISSIPPI) ---
MAP_EXTENT = [-91.8, -87.9, 29.8, 35.2] # [lon_min, lon_max, lat_min, lat_max]
CITIES = {
    "Jackson": (32.3112, -90.0759, "KJAN"),
    "Gulfport": (30.4519, -89.0703, "KGPT"),
    "Biloxi": (30.4104, -88.9244, "KBIX"), 
    "Tupelo": (34.2694, -88.7694, "KTUP"),
    "Meridian": (32.3325, -88.7517, "KMEI"),
    "Hattiesburg": (31.4678, -89.3367, "KHBG"),
    "Greenville": (33.4839, -90.9856, "KGLH"),
    "Olive Branch": (34.9392, -89.7828, "KOLV"),
}
CITY_OFFSETS = {
    "Jackson": (0.1, 0.1),
    "Hattiesburg": (0.1, -0.1),
    "Gulfport": (-0.30, 0.0), # <-- Increased left offset
    "Biloxi": (0.30, 0.0),   # <-- Increased right offset
}
SILENT_STATIONS = {
    # Border stations for "pinning" the edges
    "Memphis (TN)": (35.0425, -89.9767, "KMEM"),
    #"Huntsville (AL)": (34.6433, -86.7750, "KHSV"),
    "Mobile (AL)": (30.6869, -88.2428, "KMOB"),
    "New Orleans (LA)": (29.9934, -90.2592, "KMSY"),
    "Baton Rouge (LA)": (30.5332, -91.1496, "KBTR"),
    "Monroe (LA)": (32.5108, -92.0375, "KMLU"),
    "El Dorado (AR)": (33.2208, -92.8136, "KELD"),
    "Helena (AR)": (34.5028, -90.6386, "KHEE"),
}
# ---
FIG_BG_COLOR = '#b0c4de'
LAND_COLOR = '#f5f5dc'
...
def get_nws_alerts(state='MS'):
    """Fetches active NWS alerts for a given state (e.g., 'MS')."""
...
def generate_mississippi_weather_map(): # <-- [CHANGED] No longer async
    """Generate and save the Mississippi weather map."""
    
    logging.info("Generating Mississippi weather map...")
    
    ALL_STATIONS_MS = CITIES.copy()
    ALL_STATIONS_MS.update(SILENT_STATIONS)
    ...
    metar_cache_file = "metar_ms_cache.json"
    ...
    if metar_results is None:
        ...
        metar_results = asyncio.run(fetch_all_metars_async(ALL_STATIONS_MS))
    ...
    alert_cache_file = "alerts_ms_cache.json"
    alerts = load_json_cache(alert_cache_file, CACHE_DURATION_SECONDS)
    
    if alerts is None:
        logging.info("Fetching NWS alerts for Mississippi...")
        alerts = get_nws_alerts('MS')
    ...
    for city, temp in all_temperatures.items():
        lat, lon, _ = ALL_STATIONS_MS[city]
    ...
    fig.suptitle(f"Mississippi - Detailed Weather Map\nGenerated: {current_time}",
    ...
    clip_patch = None
    try:
        logging.info("Creating state clipping path for Mississippi...")
        target_states = ['Mississippi']
        state_geometries = []
        # --- Use 50m for speed ---
        ...
        if not state_geometries:
            raise ValueError("Could not find Mississippi in shpreader.")
        combined_geom = unary_union(state_geometries)
        if not combined_geom:
            raise ValueError("Unary union of Mississippi geometry failed.")
        paths = geos_to_path(combined_geom)
        if not paths:
            raise ValueError("geos_to_path returned no paths for Mississippi.")
        ...
        if clip_patch:
            ...
            raise ValueError("PathPatch creation failed for Mississippi.")
    except Exception as e:
        logging.error(f"Failed to create clip patch for Mississippi. Error: {e}")
        clip_patch = None
    if not clip_patch:
    ...
    for city, temp in all_temperatures.items():
        base_lat, base_lon, icao = ALL_STATIONS_MS[city]
    ...
    # --- Main Temp Colorbar (Already correct) ---
    divider = make_axes_locatable(ax)
    ...
    # --- *** LEGEND NOT MOVED (per user request) *** ---
    leg = ax.legend(handles=legend_handles, loc='lower left', fontsize=FONT_SIZE_LEGEND,
                    bbox_to_anchor=(0.01, 0.01), facecolor='white', framealpha=0.9,
    ...
    wwa_text = "### 🔔 Active Weather Alerts in Mississippi:\n"
    if alerts:
        event_groups = {}
        for alert in alerts:
            evt = alert['event']
        ...
        wwa_text = "✅ No active weather alerts for Mississippi at this time."
 ...
 # --- *** NEW DUAL-SAVE LOGIC *** ---
    ...
    dynamic_filename = f"ms_detailed_weather_{timestamp_str}.png"
    static_filename = "ms_detailed_weather.png" # For the GUI preview
...
if __name__ == "__main__":
    ...
    """
    try:
        generate_mississippi_weather_map() # <-- [CHANGED]

...
# === MODIFICATION: Moved Peachtree City to CITIES ===
SILENT_STATIONS = {
    # In-state fill-ins
    # "Peachtree City": (33.3575, -84.5714, "KFFC"), # <-- MOVED TO CITIES
    "Cartersville": (34.1231, -84.8561, "KVPC"),
    # "Winder": (34.020, -83.676, "KWDR"), # <-- EXCLUDED: Known bad data (stuck at 70°F)
    ...
    #"Chattanooga (TN)": (35.0353, -85.2038, "KCHA"),
    #"Asheville (NC)": (35.4361, -82.5417, "KAVL"),
    #"Tallahassee (FL)": (30.3967, -84.3503, "KTLH"),
    #"Jacksonville (FL)": (30.4942, -81.6878, "KJAX"),
    #"Dothan (AL)": (31.3214, -85.4494, "KDHN"),
    #"Montgomery (AL)": (32.3011, -86.3939, "KMGM"),
}
# --- CUSTOM CITY LABEL LAYOUTS ---
# Adjustments made to reduce label overlap and improve visibility.
# Critical fixes include separating Atlanta/Marietta and Macon/Robins AFB.
# Coastal cities pulled WEST (Inland) to stay visible within the -80.7 border.
CUSTOM_LAYOUTS = {
    # --- NORTH GEORGIA ---
    "Rome":          {'pos': (-85.8, 33.8), 'ha': 'right'}, # Adjusted slightly North
    "Dalton":        {'pos': (-85.4, 35.1), 'ha': 'right'},
    "Gainesville":   {'pos': (-84.1, 34.6), 'ha': 'right'},
    "Toccoa":        {'pos': (-82.9, 34.9), 'ha': 'left'},
    "Athens":        {'pos': (-83.0, 34.1), 'ha': 'left'},
    
    # *** CRITICAL FIX: Separate Marietta (NW) and Atlanta (SE) ***
    "Atlanta":       {'pos': (-84.1, 33.6), 'ha': 'left'},  # Moved Southeast to clear congestion
    "Marietta":      {'pos': (-85.3, 34.4), 'ha': 'right'}, # Moved Northwest/Up

    # --- CENTRAL GEORGIA ---
    # *** CRITICAL FIX: Separate Macon (North) and Robins (South) ***
    "Macon":         {'pos': (-83.8, 33.0), 'ha': 'right'}, # Pushed North/West
    "Robins AFB":    {'pos': (-83.1, 32.1), 'ha': 'left'},  # Pushed South/East
    
    "Columbus":      {'pos': (-85.6, 32.6), 'ha': 'right'},
    "Augusta":       {'pos': (-81.8, 33.6), 'ha': 'left'},  # Pushed East towards SC border
    "Milledgeville": {'pos': (-83.1, 33.3), 'ha': 'center'},

    # --- SOUTH GEORGIA & COAST ---
    # *** CRITICAL FIX: Vertical separation for Coast ***
    "Savannah":      {'pos': (-81.6, 31.8), 'ha': 'right'}, # Pulled South/East
    "Statesboro":    {'pos': (-82.6, 32.9), 'ha': 'right'}, # Pushed North/West to clear SAV
    
    "Brunswick":     {'pos': (-81.4, 31.0), 'ha': 'left'},  # Pushed towards Ocean
    "Waycross":      {'pos': (-83.1, 30.9), 'ha': 'right'}, # Pushed West (Inland)
    
    "Albany":        {'pos': (-85.1, 31.7), 'ha': 'right'}, # Pushed West towards AL border
    "Tifton":        {'pos': (-83.6, 31.5), 'ha': 'right'},
    "Valdosta":      {'pos': (-83.3, 30.5), 'ha': 'center'}, # Lowered slightly
}
...
def add_satellite_overlay(fig, ax, clip_patch, text_outline, bt, x_coords, y_coords, sat_proj):
    ...
    try:
        ...
        contour_bt = ax.contourf(x, y, bt, levels=levels, cmap=cmap_bt, 
                                 alpha=0.7, transform=sat_proj, zorder=5.0, extend='both', transform_first=True)
        
        # Plot Contour Lines
        contour_lines = ax.contour(x, y, bt, levels=levels, 
                                   colors='dimgrey', linewidths=0.7,
                                   alpha=0.6, transform=sat_proj, zorder=5.1)
...
def generate_georgia_weather_map():
    ...
    if ncom_lat is None:
        ...
        print(f">> Plotting Marine Layer... (Vorticity + Streamlines)")
        
    if ncom_lat is not None and grid_z is not None:
        try:
            logging.info("Plotting Marine Streamlines & Vorticity...")
            
            # 1. Grid Alignment
            nc_x, nc_y = np.meshgrid(ncom_lon, ncom_lat)
            
            # Calculate Vorticity
            marine_vort, marine_flux, marine_temp_c = calc_marine_physics(
                ncom_temp_k, ncom_u, ncom_v, 
                70, 60, 10 
            )

            # 2. NAVY STYLE 1: Vorticity Contours
            vort_levels = [-0.0001, -0.00005, 0.00005, 0.0001] 
            ax.contour(nc_x, nc_y, marine_vort, levels=vort_levels, 
                       colors=['blue', 'cyan', 'orange', 'red'], linewidths=0.8, 
                       alpha=0.6, transform=ccrs.PlateCarree(), zorder=1.56)

            # 3. NAVY STYLE 2: Hurricane Fuel Line (26C)
            ax.contour(nc_x, nc_y, marine_temp_c, levels=[26.0], 
                       colors=['#FF0000'], linewidths=2.0, linestyles='dashed',
                       transform=ccrs.PlateCarree(), zorder=1.57)
            
            # 4. STREAMLINES
            magnitude = np.sqrt(ncom_u**2 + ncom_v**2)
            
            strm = ax.streamplot(ncom_lon, ncom_lat, ncom_u, ncom_v, 
                                 color=ncom_sal, cmap='viridis', 
                                 density=2.0, 
                                 linewidth=1.0, 
                                 arrowsize=0.8,
                                 transform=ccrs.PlateCarree(), 
            ...
            # --- FIX IS HERE ---
            if clip_patch:
                # Apply clip path to the Collections directly (No loop!)
    ...
    if grid_p is not None:
        try:
            cs_iso_fine = ax.contour(grid_px, grid_py, grid_p, levels=np.arange(900, 1100, 0.5), 
                                     colors="#0E2914", linewidths=1.5, alpha=0.9, zorder=3.5, transform=ccrs.PlateCarree())
            ...
            clabels_iso = ax.clabel(cs_iso_label, inline=True, fontsize=10, fmt='%1.0f', colors='white', use_clabeltext=True, zorder=3.6)
        ...
        for rec in reader.records():
            if rec.attributes.get('type') in ['Major Highway', 'Secondary Highway']:
                ...
                if (b[0] <= MAP_EXTENT[1] and b[2] >= MAP_EXTENT[0] and b[1] <= MAP_EXTENT[3] and b[3] >= MAP_EXTENT[2]):
                    ...
                    p = ax.add_geometries([rec.geometry], ccrs.PlateCarree(), facecolor='none', edgecolor=color, linewidth=lw, zorder=2.5, alpha=1.0)
    ...
    def get_text_props(color_hex, fsize=12, is_bold=True):
        return dict(
            ...
            bbox=dict(boxstyle="round,pad=0.3,rounding_size=0.2", facecolor='white', edgecolor=color_hex, linewidth=2.5, alpha=0.95)
        )
    
    props_temp = get_text_props('#FF0000', 15) # Red Glow
    props_dew  = get_text_props('#32CD32', 12) # Green Glow
    props_alt  = get_text_props('#4682B4', 12) # Blue Glow
    props_cond = get_text_props('#555555', 12, is_bold=False) # Gray Glow
    
    # Footer props (Pill)
    pill_bbox = dict(boxstyle="round,pad=0.2,rounding_size=0.8", facecolor='#555555', edgecolor='none', alpha=0.9)
    props_footer = dict(color='white', fontsize=11, fontweight='bold', 
                        path_effects=[pe.withStroke(linewidth=2.5, foreground='black'), pe.Normal()], bbox=pill_bbox)
    ...
    for icao, data in all_station_data.items():
        ...
        # Draw Leader Line
        leader_line, = ax.plot([base_lon, box_x], [base_lat, box_y], color='black', ls=':', lw=2.5, zorder=25,
        ...
        # --- A. CITY NAME BOX ---
        # Determine style (Glow vs Standard)
        if city_name in GLOW_CITIES:
            name_props = dict(
                color='white', fontsize=13, fontweight='bold',
                path_effects=[pe.withStroke(linewidth=5, foreground="#FF0044", alpha=0.8), 
                              pe.withStroke(linewidth=1.5, foreground='black'), pe.Normal()],
                bbox=dict(boxstyle="round,pad=0.3,rounding_size=0.2", facecolor='black', 

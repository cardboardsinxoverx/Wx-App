...
# -*- coding: utf-8 -*-
"""
Standalone California (CA) Weather Map Generator.
...
NEXRAD radar image to generate a detailed weather map for the state of California.

The final map is saved as 'ca_detailed_weather.png'.
"""
...
# --- Configuration (CALIFORNIA) ---
MAP_EXTENT = [-125.0, -114.0, 32.5, 42.2] # [lon_min, lon_max, lat_min, lat_max]
CITIES = {
    "Los Angeles": (33.942, -118.408, "KLAX"),
    "San Francisco": (37.619, -122.375, "KSFO"),
    "San Diego": (32.733, -117.189, "KSAN"),
    "Sacramento": (38.695, -121.590, "KSMF"),
    "Fresno": (36.776, -119.718, "KFAT"),
    "Redding": (40.509, -122.293, "KRDD"),
    "Bakersfield": (35.433, -119.056, "KBFL"),
    "Palm Springs": (33.829, -116.506, "KPSP"),
    "Eureka/Arcata": (40.978, -124.108, "KACV"),
    "Monterey": (36.586, -121.842, "KMRY"),
    "Santa Barbara": (34.426, -119.840, "KSBA"),
}
CITY_OFFSETS = {
    "Los Angeles": (0.1, 0.1),
    "San Francisco": (0.1, 0.1),
    "San Diego": (0.1, 0.1),
    "Eureka/Arcata": (0.1, 0.1),
    "Monterey": (0.1, 0.1),
    "Santa Barbara": (0.1, 0.1),
}
SILENT_STATIONS = {
    # In-state fill-ins
    "Bishop": (37.373, -118.363, "KBIH"),
    "Ukiah": (39.126, -123.201, "KUKI"),
    "Truckee": (39.319, -120.138, "KTRK"),
    
    # Border stations for "pinning" the edges
    # Oregon (North)
    "Medford (OR)": (42.374, -122.873, "KMFR"),
    "Klamath Falls (OR)": (42.155, -121.731, "KLMT"),
    
    # Nevada (East)
    "Reno (NV)": (39.499, -119.768, "KRNO"),
    "Las Vegas (NV)": (36.080, -115.152, "KLAS"),
    
    # Arizona (East/Southeast)
    "Phoenix (AZ)": (33.434, -112.011, "KPHX"),
    "Yuma (AZ)": (32.656, -114.606, "KNYL"),
    "Page (AZ)": (36.923, -111.449, "KPGA"),
    
    # Mexico (South)
    "Tijuana (MX)": (32.541, -116.983, "MMTJ"),
}
# --- *** CORRECTION: Renamed variable *** ---
ALL_STATIONS_CA = {**CITIES, **SILENT_STATIONS}
# ---
FIG_BG_COLOR = '#b0c4de'
LAND_COLOR = '#f5f5dc'
OCEAN_COLOR = '#b0e0e6'
...
def add_satellite_overlay(fig, ax, clip_patch, text_outline, bt, x_coords, y_coords, sat_proj):
    ...
    try:
        ...
        if clip_patch:
            # We don't clip this layer
            pass
                
        # --- Draw colorbar using make_axes_locatable *** ---
...
# --- *** START: REPLACED PLACEHOLDER FUNCTIONS *** ---
# These functions are copied from ga_wx.py to replace the broken placeholders
...
def is_daytime(lat, lon):
    ...
    try:
        # Use timezone.utc for all calculations
...
def get_nws_alerts(state='CA'):
    """Fetches active NWS alerts for a given state (e.g., 'CA')."""
...
# --- *** END: REPLACED PLACEHOLDER FUNCTIONS *** ---


# --- Main map generation function ---
def generate_california_weather_map(): # <-- *** CORRECTION: Renamed function ***
    """Main function to generate the California weather map."""
    ...
    metar_cache_file = "metar_ca_cache.json"
    ...
    if metar_results is None:
        ...
        metar_results = asyncio.run(fetch_all_metars_async(ALL_STATIONS_CA))
    ...
    # --- *** END: Replaced async logic *** ---

    # --- NWS ALERTS WITH CACHE ---
    alert_cache_file = "alerts_ca_cache.json"
    alerts = load_json_cache(alert_cache_file, CACHE_DURATION_SECONDS)
    
    if alerts is None:
        logging.info("Fetching NWS alerts for California...")
        alerts = get_nws_alerts('CA') # <-- *** CORRECTION: Changed to CA ***
    ...
    for city, temp in all_temperatures.items():
        lat, lon, _ = ALL_STATIONS_CA[city] # <-- *** CORRECTION: Changed to CA ***
    ...
    fig = plt.figure(figsize=(16, 16), facecolor=FIG_BG_COLOR) # Adjusted size
    
    text_outline = [pe.withStroke(linewidth=3, foreground='white')]
    
    current_time = datetime.now().strftime('%Y-%m-%d %I:%M %p') 
    fig.suptitle(f"California - Detailed Weather Map\nGenerated: {current_time}",
                 fontsize=FONT_SIZE_TITLE, fontweight="bold", y=0.88, color="navy",
                 path_effects=text_outline) 

    ax = fig.add_subplot(1, 1, 1, projection=ccrs.PlateCarree())
    ax.set_extent(MAP_EXTENT, crs=ccrs.PlateCarree())
    
    # --- Robust STATE CLIP PATCH ---
    clip_patch = None
    try:
        logging.info("Creating state clipping path for California...")
        target_states = ['California'] 
        state_geometries = []
        # Use 50m data for speed
        ...
        if not state_geometries:
            raise ValueError("Could not find California in shpreader.")
        
        # Combine geometries
        combined_geom = unary_union(state_geometries)
        if not combined_geom:
            raise ValueError("Unary union of California geometry failed.")
            
        # Convert the Shapely geometry to a Matplotlib Path
        paths = geos_to_path(combined_geom)
        if not paths:
            raise ValueError("geos_to_path returned no paths for California.")
        ...
        if clip_patch:
            ax.add_patch(clip_patch)  # Add the patch to the axes
        else:
            raise ValueError("PathPatch creation failed for California.")
    except Exception as e:
        logging.error(f"Failed to create clip patch for California. Error: {e}")
        clip_patch = None
    ...
    # Temperature Contours (zorder 1.5)
    if grid_z is not None and grid_lon is not None and grid_lat is not None:
        cmap = cm.get_cmap('jet')
        boundaries = np.arange(-30, 122, 2) # 2-degree steps
    ...
    if grid_u is not None and grid_v is not None and grid_lon is not None and grid_lat is not None:
        s = 7 # Plot a barb every 7th grid point
    ...
    try:
        ...
        county_patches = ax.add_feature(counties_feature, 
                       ...
                       alpha=0.8)
        if clip_patch:
            # Clip the county lines to the state border
    ...
    # Colorbar setup
    cmap = cm.get_cmap('jet')
    boundaries = np.arange(-30, 122, 2) 
    norm = BoundaryNorm(boundaries=boundaries, ncolors=cmap.N, extend='both')
    sm = plt.cm.ScalarMappable(cmap=cmap, norm=norm)
    sm.set_array([])

    # Plot city/station data
    logging.info("Plotting station data...")
    for city, temp in all_temperatures.items():
        base_lat, base_lon, icao = ALL_STATIONS_CA[city] # <-- *** CORRECTION: Changed to CA ***
        
        # CITIES are the main, labeled stations
        if city in CITIES:
            ...
            # Station scatter point, colored by temp
            if temp is not None:
            ...
            if condition:
                ...
                if condition in WEATHER_ICONS:
                    try:
                        ...
                        # Pack icon and text together horizontally
                        packer = HPacker(
        ...
        # SILENT_STATIONS are small, unlabeled points
        else:
            if temp is not None:
                color = cmap(norm(temp))
    ...
    for alert in alerts:
        for polygon_coords in alert['polygons']:
            ...
            # Create a legend handle for this alert type if it doesn't exist
    ...
    cbar.set_label('Temperature (°F)', fontsize=FONT_SIZE_LEGEND, fontweight='bold', color='navy',
                   path_effects=text_outline)
    cbar.set_ticks(np.arange(-30, 121, 10)) # Ticks every 10 degrees
    ...
    legend_handles.extend(alert_handles.values()) # Add active alerts

    leg = ax.legend(handles=legend_handles, loc='lower left', fontsize=FONT_SIZE_LEGEND,
                    bbox_to_anchor=(0.0, 0.0), facecolor='white', framealpha=0.9,
                    edgecolor='lightgray', fancybox=True, ncol=2)
    leg.get_frame().set_linewidth(0.5)
    leg.set_zorder(100) # Make sure legend is on top
    ...
    # Build the alert text string
    wwa_text = "### 🔔 Active Weather Alerts in California:\n" # <-- *** CORRECTION: Changed to CA ***
    if alerts:
        event_groups = {}
        for alert in alerts:
            evt = alert['event']
            ...
            # Add all affected areas, splitting by comma
        ...
        for event, data in event_groups.items():
            ...
            # Join all sorted areas for a clean list
            wwa_text += f"> {', '.join(sorted(list(data['areas'])))}\n"
    else:
        wwa_text = "✅ No active weather alerts for California at this time." # <-- *** CORRECTION: Changed to CA ***

    # Print the text output to the console
    print("\n" + "="*30)
    print(wwa_text.replace("### 🔔 ", "").replace("**", "").replace("> ", "")) # Clean markdown for console
    print("="*30 + "\n")
...
# --- *** NEW DUAL-SAVE LOGIC *** ---
    ...
    dynamic_filename = f"ca_detailed_weather_{timestamp_str}.png" # <-- *** CORRECTION: Changed to CA ***
    static_filename = "ca_detailed_weather.png" # For the GUI preview # <-- *** CORRECTION: Changed to CA ***
...
if __name__ == "__main__":
    ...
    try:
        generate_california_weather_map() # <-- *** CORRECTION: Renamed function ***

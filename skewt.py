...
async def fetch_bufkit_data_async(ctx, station_code, model, forecast_hour, max_retries=10, retry_delay=2):
    ...
    # Use the current date for the BUFKIT URL
    current_date = datetime.datetime.now(pytz.UTC)
    year, month, day = current_date.year, current_date.month, current_date.day
    # Try both the current and previous day to handle model run availability
    for date_offset in [0, -1]:
        date = current_date + datetime.timedelta(days=date_offset)
        year, month, day = date.year, date.month, date.day
        url = f"https://mtarchive.geol.iastate.edu/{year}/{month:02d}/{day:02d}/bufkit/{model_dir}_{nws_station.lower()}.buf"
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    await ctx.send(f"HTTP error fetching BUFKIT data: {str(e)}. Retrying in {retry_delay}s... (Attempt {attempt + 1}/{max_retries})")
                ...
                if attempt < max_retries - 1:
                    ...
                    await ctx.send(f"Failed to fetch BUFKIT data for {station_code} after {max_retries} attempts: {str(e)}")
                    return None, None
    await ctx.send(f"No BUFKIT data found for {station_code} ({nws_station}) with model {model} for the current or previous day.")
    return None, None
...
async def skewt(ctx, *args):
    ...
    **Usage:**
    ...
    - **Forecast Sounding:** `$skewt <station> <model> <forecast_hour>`
      ...
      - `<forecast_hour>`: Integer (0-384 for GFS/GFSM, 0-84 for NAM/NAMM, 0-18 for RAP)
    ...
    **Notes:**
    ...
    try:
        ...
        # Add storm hazard text (using the single determine_storm_hazard function)
        storm_hazard = determine_storm_hazard(
            sbcape, total_helicity1, lcl_height, bshear6_mag, mucape, PWAT, surface_RH
        )

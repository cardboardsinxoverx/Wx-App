...
"""
import warnings
# Silence coordinate and metadata warnings from Rasterio and Cartopy
...
try:
    ...
    logging.warning("Herbie not found. RRFS data will be unavailable.")
import time
import xarray as xr
...
# --- NEW: Weather Icon File List (From Image) ---
WEATHER_ICON_FILES = [
    "blowingsnow@2x.png", "clear@2x.png", "clearn@2x.png", "cloudy@2x.png",
    "drizzle@2x.png", "drizzlen@2x.png", "dust@2x.png", "dustn@2x.png",
    "fair@2x.png", "fairn@2x.png", "fdrizzle@2x.png", "fdrizzlen@2x.png",
    "flurries@2x.png", "flurriesn@2x.png", "fog@2x.png", "fogn@2x.png",
    "freezingrain@2x.png", "hazy@2x.png", "hazyn@2x.png", "mcloudy@2x.png",
    "mcloudyn@2x.png", "mcloudyr@2x.png", "mcloudyrn@2x.png", "mcloudys@2x.png",
    "mcloudysf@2x.png", "mcloudysfn@2x.png", "mcloudysn@2x.png", "mcloudyt@2x.png",
    "mcloudytn@2x.png", "na@2x.png", "pcloudy@2x.png", "pcloudyn@2x.png",
    "pcloudyr@2x.png", "pcloudyrn@2x.png", "pcloudys@2x.png", "pcloudysf@2x.png",
    "pcloudysfn@2x.png", "pcloudysn@2x.png", "pcloudyt@2x.png", "pcloudytn@2x.png",
    "rain@2x.png", "rainandsnow@2x.png", "rainn@2x.png", "raintosnow@2x.png",
    "raintosnown@2x.png", "showers@2x.png", "showersn@2x.png", "sleet@2x.png",
    "sleetn@2x.png", "sleetsnow@2x.png", "sleetsnown@2x.png", "smoke@2x.png",
    "smoken@2x.png", "snow@2x.png", "snowflurries@2x.png", "snown@2x.png",
    "snowshowers@2x.png", "snowshowersn@2x.png", "snowtorain@2x.png", "snowtorainn@2x.png",
    "sunny@2x.png", "sunnyn@2x.png", "tstorm@2x.png", "tstormn@2x.png",
    "wintrymix@2x.png", "wintrymixn@2x.png"
]
...
def log_status(source_name, hit=False, success=True):
    ...
    # Track specific source activity
...
# --- REPLACEMENT CODE ---
# We are skipping the old matplotlib icon generation because we use PNGs now.
WEATHER_ICONS = {}
logging.info("Skipped legacy icon generation (Using local PNG icons instead).")
...
TEMP_ALERT_COLORS = {
    ...
    "Cold Weather Advisory": "#4C94C0", # Light Blue
    "Wind Chill Advisory": "#87CEFA",   # Added as a fallback/synonym
}
...
def draw_broadcast_header(fig, main_title, sub_title, right_badge_text="LIVE", 
    ...
    logo_ax.axis('off')
    # Inside draw_broadcast_header
    logo_path = "/media/evan/Main/frostbyte/frostbyte_project/icons/boxlogo4.png"
    if os.path.exists(logo_path):
        logo_img = plt.imread(logo_path)
        logo_ax.imshow(logo_img)
    else:
        print(f"CRITICAL: Logo not found at {logo_path}")
    try:
        logo_img = plt.imread(logo_path)
        logo_ax.imshow(logo_img)
    except Exception:
...
CITIES = {
    ...
    # "Clayton": (34.8781, -83.4011, "KJZP"), dont work, error 204 whatever that means
    "Toccoa":        (34.5936, -83.2961, "KTOC"), # Reliable NE GA replacement
    "Valdosta": (30.8327, -83.2785, "KVLD"),
    "Albany": (31.5785, -84.1557, "KABY"),
    "Waycross": (31.2136, -82.3549, "KAYS"),
    "Robins AFB": (32.6400, -83.5917, "KWRB"),
    "Augusta": (33.3699, -81.9625, "KAGS"),
    "Macon": (32.6934, -83.6489, "KMCN"),
    "Columbus": (32.5161, -84.9389, "KCSG"),
    "Brunswick": (31.1518, -81.3911, "KBQK"),
    "Statesboro": (32.4735, -81.5629, "KTBR"),
    "Marietta": (33.9153, -84.5498, "KMGE"),
    "Peachtree City": (33.3575, -84.5714, "KFFC"), # <-- MOVED FROM SILENT
    "Tifton":         (31.4504, -83.5085, "KTMA"),
     # "Milledgeville":  (33.1536, -83.2406, "KMLJ")
}

# --- SILENT STATIONS (Small Dot + ICAO Only) ---
SILENT_STATIONS = {
    # Georgia Internals
    "Cartersville": (34.1231, -84.8561, "KVPC"),
    "Dublin":       (32.559, -82.986, "KDBN"),
    "Alma":         (31.536, -82.515, "KAMG"),
    "Camilla":      (31.229, -84.148, "KCXU"),
    "Vidalia":      (32.209, -82.373, "KVIH"),
    "Cornelia":     (34.5, -83.5, "KHAB"), # Habersham Co
    "LaGrange":     (33.0, -85.0, "KLGC"),
    
    # Border / Pinning Stations (Crucial for visual framing)
    "Chattanooga":  (35.0353, -85.2038, "KCHA"),
    "Tallahassee":  (30.3967, -84.3503, "KTLH"),
    "Jacksonville": (30.4942, -81.6878, "KJAX"),
    "Dothan":       (31.3214, -85.4494, "KDHN"),
    "Montgomery":   (32.3011, -86.3939, "KMGM"),
    "Anderson":     (34.5, -82.7, "KAND"), # SC
}

# --- UPDATED LAYOUTS (Macon/Robins Collision Fix) ---
CUSTOM_LAYOUTS = {
    # --- NORTH GEORGIA ---
    "Rome":          {'pos': (-85.32, 34.18), 'ha': 'right'},
    "Dalton":        {'pos': (-85.15, 34.90), 'ha': 'right'},
    "Gainesville":   {'pos': (-83.95, 34.45), 'ha': 'right'},
    "Toccoa":        {'pos': (-83.85, 35.16), 'ha': 'right'},
    "Athens":        {'pos': (-83.15, 34.05), 'ha': 'left'},
    
    "Atlanta":       {'pos': (-84.20, 33.60), 'ha': 'left'},
    "Marietta":      {'pos': (-85.45, 33.60), 'ha': 'right'},
    ...
    "Albany":        {'pos': (-85.15, 31.60), 'ha': 'right'},
...
def fetch_and_plot_glm_l2_lcfa(ax, timestamp_outline):
    """
    Fetches GOES-16 Level 2 LCFA Data.
    V31 FIX: Explicitly closes datasets to prevent OPeNDAP socket hang on function exit.
    """
    ...
    print("\n" + "="*40)
    print(" ⚡ STARTING GLM LEVEL 2 FETCH (V31 - FAST CLOSE) ")
    print("="*40)
    logging.info("Fetching GOES-16 L2 LCFA Data...")

    # --- 1. SETUP DATES ---
    now = datetime.now(timezone.utc)
    # Only try TODAY. Yesterday is irrelevant for "live" lightning.
    # If today's folder doesn't exist (00:00 UTC edge case), we skip it.
    dates_to_try = [now] 
    
    base_url_template = 'https://thredds.ucar.edu/thredds/catalog/satellite/goes/east/grb/GLM/LCFA/{date_str}/catalog.xml'
    ...
    for date_obj in dates_to_try:
        ...
        catalog_url = base_url_template.format(date_str=date_str)
        print(f"• Checking: {catalog_url} ...")
        
        try:
            cat = TDSCatalog(catalog_url)
            
            # Handle Siphon's dataset dictionary
            if hasattr(cat.datasets, 'values'):
                ds_iterable = cat.datasets.values()
            else:
                ds_iterable = cat.datasets
            
            datasets = [d for d in ds_iterable if 'LCFA' in d.name and '.nc' in d.name]
            
            if not datasets:
                print(f"  - No .nc files found.")
                continue
                
            # Get 3 most recent files (reduced from 5 to speed up processing)
            datasets.sort(key=lambda x: x.name)
            recent_files = datasets[-3:]
            print(f"• Found {len(recent_files)} recent files. Downloading...", end=" ", flush=True)
            
            found_data = True 
            count = 0
            
            # --- 3. PROCESS FILES (WITH EXPLICIT CLOSE) ---
            for ds_ref in recent_files:
                try:
                    # Open dataset
                    # We use a context manager logic manually or explicit close
                    ds_access = ds_ref.remote_access(service='OPENDAP')
                    ds = xr.open_dataset(ds_access)
                    ...
                    try:
                        if 'flash_lat' in ds and 'flash_lon' in ds:
                            ...
                            # Filter to GA Extent
                            mask = (lons >= -86.8) & (lons <= -80.7) & (lats >= 30.2) & (lats <= 35.8)
                            
                            if np.any(mask):
                                flash_lats.extend(lats[mask])
                                flash_lons.extend(lons[mask])
                                count += np.sum(mask)
                    finally:
                        # CRITICAL: Force close the network connection immediately
                        ds.close()
                        
                except Exception:
                    continue
            
            print(f"Done. (Extracted {count} flashes)")

        except Exception as e:
            if "404" in str(e): print(f"  - 404 Not Found.")
            else: print(f"  - Error: {e}")
            continue

    # --- 4. PLOT ---
    if flash_lats:
        print(f"• Plotting {len(flash_lats)} flashes...")
        ax.scatter(flash_lons, flash_lats, marker='P', s=60, c='#FFD700',
                   edgecolors='black', linewidth=0.8, transform=ccrs.PlateCarree(),
                   zorder=105, label='GLM Flashes', path_effects=timestamp_outline)
        print("⚡ GLM Success.")
    else:
        print("⚡ GLM Complete: No lightning detected.")
...
def add_radar_image_overlay(fig, ax, cache_duration, text_outline, clip_patch=None, rtma_data=None, hrrr_data=None):
    ...
    FIXED: Resolves the truth-value ambiguity error during transformation checks.
    """
    source_label = "NEXRAD Radar"
    RADAR_CACHE_TTL = 600
    ...
    try:
        if raw_bytes[:2] == b'\x1f\x8b': 
            ...
            decompressed_data = unlzw3.unlzw(raw_bytes)
        
        with io.BytesIO(decompressed_data) as memfile:
            with rasterio.Env(NOT_GEOREFERENCED_WARNING='OFF'):
                with rasterio.open(memfile) as src:
                    # FIXED: Use .all() to handle array returns from is_identity
                    transform_is_empty = np.all(src.transform.is_identity)
                    
                    if transform_is_empty or src.bounds.left == 0.0:
                        raw_data = src.read(1)
        ...
        dbz_data = gaussian_filter(dbz_data, sigma=1.5)
        dbz_masked = np.ma.masked_where(dbz_data < 5, dbz_data)
        
        radar_img = ax.imshow(dbz_masked, origin='upper', extent=precise_extent, 
                              transform=ccrs.PlateCarree(),
                              cmap=ctables.registry.get_colortable('NWSReflectivity'), 
                              vmin=5, vmax=80, zorder=4.0, alpha=0.75)
        
        if clip_patch:
            radar_img.set_clip_path(clip_patch)
            
        logging.info("Radar overlay successfully plotted.")
    except Exception as e:
        logging.error(f"Critical error in radar processing: {e}")
    ...
    try:
        ...
        for feature in features:
            try:
                ...
                elif "Freezing Fog" in event: color = "#008080" # Teal/Green for Freezing Fog
                elif "Fog" in event: color = "#808080"  # Gray for standard Dense Fog
                # *** UPDATED SECTION END ***
...
def _resolve_text_collisions(fig, ax, items, padding_px=2, max_iter=1000, pixel_step=2):
    """
    Iterative solver with a 'Leash'.
    Moves text small amounts (2px) and refuses to move them if they
    wander more than 40px from their original spot.
    """
    if not items:
        return
    ...
    # 2. Store original positions (The Leash Anchor)
    # We map the artist object to its starting (x, y)
    origins = {}
    for entry in items:
        artist = entry['item']
        if isinstance(artist, AnnotationBbox):
            origins[artist] = artist.xy
        else:
            origins[artist] = artist.get_position()

    MAX_DIST = 40.0 # Maximum pixels an item can wander

    # 3. Helper to get boxes
    def get_box(item):
        ...
        for i in range(len(items)):
            for j in range(i + 1, len(items)):
                ...
                if boxes_overlap(b1, b2, pad=padding_px):
                    ...
                    # Target index and direction
                    targets = [] # List of (index, dir_x, dir_y)
                    
                    if item_a.get('fixed', False):
                        targets.append((j, -ux, -uy)) # Move B away
                    elif item_b.get('fixed', False):
                        targets.append((i, ux, uy))   # Move A away
                    else:
                        targets.append((i, ux/2, uy/2))   # Move A half
                        targets.append((j, -ux/2, -uy/2)) # Move B half

                    # Apply moves ONLY if within Leash
                    for idx, dir_x, dir_y in targets:
                        artist = items[idx]['item']
                        
                        # Calculate proposed move
                        # Convert pixel step to data coordinates approx
                        # Note: This is rough, but effective enough for repulsion
                        # We use the inverse transform to get a delta
                        
                        # Current pos
                        if isinstance(artist, AnnotationBbox): cur_pos = artist.xy
                        else: cur_pos = artist.get_position()

                        # Get shift in data coords
                        p0 = inv.transform((0, 0))
                        p1 = inv.transform((dir_x * pixel_step, dir_y * pixel_step))
                        d_data_x, d_data_y = p1[0] - p0[0], p1[1] - p0[1]
                        
                        prop_x = cur_pos[0] + d_data_x
                        prop_y = cur_pos[1] + d_data_y

                        # CHECK LEASH (Distance from origin)
                        orig_pos = origins[artist]
                        # We compare in Data coords (rough approximation of distance)
                        dist_sq = (prop_x - orig_pos[0])**2 + (prop_y - orig_pos[1])**2
                        
                        # 40 pixels in map coords is roughly 0.5 degrees, adjust as needed
                        # Or just check simple boundary:
                        if dist_sq < 0.5: # 0.5 degree leash (approx 35 miles)
                            _apply_move(artist, (d_data_x, d_data_y))
                            moved = True
...
def fetch_rap_full(extent):
    """
    Fetches FULL RAP Data: Temp, Dew, Wind, AND Pressure (MSLP).
    """
    logging.info("Fetching RAP Surface Analysis (+MSLP)...")
    try:
        buffer = 1.0
        bbox = [extent[2]-buffer, extent[3]+buffer, extent[0]-buffer, extent[1]+buffer] 
        ...
        dew_var = find_var(['Dewpoint_temperature_height_above_ground', 'Dewpoint_temperature_surface', 'dpt2m'])
        rh_var  = find_var(['Relative_humidity_height_above_ground', 'Relative_humidity_surface', 'rh2m'])
        ...
        # --- NEW: PRESSURE ---
        pres_var = find_var(['MSLP_MAPS_System_Reduction_msl', 'Pressure_reduced_to_MSL_msl', 'prmsl'])

        if not temp_var or not u_var or not v_var: return None

        # Query
        vars_to_fetch = [temp_var, u_var, v_var]
        if dew_var: vars_to_fetch.append(dew_var)
        elif rh_var: vars_to_fetch.append(rh_var)
        if pres_var: vars_to_fetch.append(pres_var)
        ...
        temp_f = (get_val(temp_var) - 273.15) * 9/5 + 32
        u_kts = get_val(u_var) * 1.94384
        v_kts = get_val(v_var) * 1.94384
        
        # Process Pressure (Pa -> hPa)
        p_mb = None
        if pres_var:
            p_mb = get_val(pres_var) / 100.0

        # Dewpoint Logic
        if dew_var:
            dew_f = (get_val(dew_var) - 273.15) * 9/5 + 32
        elif rh_var:
            rh = get_val(rh_var)
            t_c = (temp_f - 32) * 5/9
            b = 17.67; c = 243.5
            gamma = np.log(rh / 100.0) + (b * t_c) / (c + t_c)
        ...
        logging.error(f"RAP Full Fetch Failed: {e}")
        return None

def fetch_hrrr_sfc(extent):
    """
    Fetches HRRR Surface Data: Temp, Dew, Wind, AND Pressure (MSLP).
    """
    logging.info("Fetching HRRR Surface Data (Operational)...")
    try:
        buffer = 0.5
        bbox = [extent[2]-buffer, extent[3]+buffer, extent[0]-buffer, extent[1]+buffer]
        ...
        d_var = find(['Dewpoint_temperature_height_above_ground', 'Dewpoint_temperature_surface'])
        ...
        # --- NEW: PRESSURE ---
        p_var = find(['MSL_pressure_surface', 'Pressure_reduced_to_MSL_msl', 'prmsl'])

        if not t_var or not u_var: return None
        ...
        vars_to_get = [t_var, d_var, u_var, v_var, p_var]
        valid_vars = [v for v in vars_to_get if v]
        query.variables(*valid_vars)
        ...
        def get(name):
            if not name: return np.zeros_like(lats)
            val = data.variables[name][:].squeeze()
            if val.ndim == 3: val = val[0]
            return val

        temp_f = (get(t_var) - 273.15) * 9/5 + 32
        dew_f  = (get(d_var) - 273.15) * 9/5 + 32
        u_kts  = get(u_var) * 1.94384
        v_kts  = get(v_var) * 1.94384
        
        # Process Pressure (Pa -> hPa)
        p_mb = None
        if p_var:
            p_mb = get(p_var) / 100.0
        
        logging.info("HRRR Surface Data fetched successfully.")
...
def blend_rap_hrrr(rap_data, hrrr_data, extent):
    """
    Blends RAP (13km) and HRRR (3km).
    Includes Pressure (MSLP) blending for high-res isobars.
    """
    logging.info("Blending RAP and HRRR models...")
    
    grid_lon = np.arange(extent[0]-0.5, extent[1]+0.5, 0.025)
    grid_lat = np.arange(extent[2]-0.5, extent[3]+0.5, 0.025)
    grid_x, grid_y = np.meshgrid(grid_lon, grid_lat)
    
    points = []
    temps, dews, us, vs, press = [], [], [], [], [] # Added press list
    
    # --- 1. RAP ---
    if rap_data:
        r_lon = rap_data['lon'].flatten()
        r_lat = rap_data['lat'].flatten()
        r_lon = np.where(r_lon > 180, r_lon - 360, r_lon)
        
        points.append(np.column_stack((r_lon, r_lat)))
        temps.append(rap_data['temp'].flatten())
        dews.append(rap_data['dew'].flatten())
        us.append(rap_data['u'].flatten())
        vs.append(rap_data['v'].flatten())
        if rap_data.get('pressure') is not None:
            press.append(rap_data['pressure'].flatten())
        logging.info("Added RAP points.")

    # --- 2. HRRR ---
    if hrrr_data:
        h_lon = hrrr_data['lon'].flatten()
        h_lat = hrrr_data['lat'].flatten()
        h_lon = np.where(h_lon > 180, h_lon - 360, h_lon)
        
        points.append(np.column_stack((h_lon, h_lat)))
        temps.append(hrrr_data['temp'].flatten())
        dews.append(hrrr_data['dew'].flatten())
        us.append(hrrr_data['u'].flatten())
        vs.append(hrrr_data['v'].flatten())
        if hrrr_data.get('pressure') is not None:
            press.append(hrrr_data['pressure'].flatten())
        logging.info("Added HRRR points.")

    if not points:
        logging.error("CRITICAL: No model data available.")
        return None, None, None, None, None, None, None # Added extra None

    # --- 3. Interpolate ---
    all_points = np.vstack(points)
    all_temps = np.concatenate(temps)
    all_dews = np.concatenate(dews)
    all_us = np.concatenate(us)
    all_vs = np.concatenate(vs)
    # Handle pressure separately in case one model was missing it
    all_press = np.concatenate(press) if press else None
    
    logging.info(f"Interpolating {len(all_points)} points...")
    from scipy.interpolate import griddata
    
    def interp_safe(vals, sigma=0.5):
    ...
    try:
        g_temp = interp_safe(all_temps)
        g_dew = interp_safe(all_dews)
        g_u = interp_safe(all_us)
        g_v = interp_safe(all_vs)
        
        # --- PRESSURE ---
        # We use a slightly higher sigma (1.0) for pressure even in "jagged" mode
        # because pure 0.5 sigma pressure contours can look like static noise.
        # 1.0 retains the kinks and troughs without looking broken.
        g_p = interp_safe(all_press, sigma=1.0) if all_press is not None else None
        
        return grid_y, grid_x, g_temp, g_dew, g_u, g_v, g_p # Added g_p
...
def fetch_ncom_data(extent):
    """
    Fetches MARINE Data via NOAA RTOFS.
    FIXED: Uses a dedicated session with User-Agent to prevent server rejection.
    """
    ...
    # CRITICAL: Create a session with a browser-like User-Agent
    session = requests.Session()
    session.headers.update({'User-Agent': 'Mozilla/5.0 WeatherMapBot/1.0'})
    ...
    for date_obj in dates_to_try:
        ...
        try:
            # Open dataset with explicit engine and session-like behavior
            ...
            lats, lons = ds_sub.lat.values, ds_sub.lon.values
            lons = np.where(lons > 180, lons - 360, lons)
            
            w_temp = ds_sub['sst'].values.squeeze()
            if np.nanmax(w_temp) < 100: w_temp += 273.15 # Ensure Kelvin
                
            w_u = ds_sub['u_velocity'].values.squeeze()
            w_v = ds_sub['v_velocity'].values.squeeze()
            salinity = ds_sub['sss'].values.squeeze() if 'sss' in ds_sub else np.zeros_like(w_temp)
...
def fetch_and_plot_wpc_pressure_centers(ax, rap_data=None):
    ...
    V2 Updated: Adds console printing to prevent "stuck" feeling.
    """
    import geopandas as gpd
    from matplotlib.offsetbox import AnnotationBbox, TextArea, OffsetImage, VPacker
    import matplotlib.patheffects as pe
    from shapely.geometry import Polygon

    print("\n" + "="*40)
    print(" 🔵 FETCHING PRESSURE CENTERS (WPC) ")
    print("="*40)
    logging.info("Fetching WPC Pressure Centers (ArcGIS Source)...")
    
    # --- CONFIGURATION ---
    # Update these paths if your icons are elsewhere
    ICON_PATHS = {
        'H': "/media/evan/Main/frostbyte/frostbyte_project/icons/high.png",
        'L': "/media/evan/Main/frostbyte/frostbyte_project/icons/low.png"
    }
    ICON_ZOOM = 0.15 
    ...
    for layer_id in layers_to_scan:
        try:
            print(f"• Scanning Layer {layer_id}...", end=" ", flush=True)
            query_url = f"{base_url}/{layer_id}/query?where=1%3D1&outFields=*&f=geojson&outSR=4326"
            
            # Reduced timeout to 3s to speed up "skips"
            ...
            if r.status_code == 200:
                ...
                if 'features' in data and data['features']:
                    ...
                    if not points.empty:
                        ...
                        print(f"Found {len(points)} items. ✅")
                    else:
                        print("No points. ⚪")
                else:
                    print("Empty. ⚪")
            ...
            print("Timeout/Error. ⚠️")
            continue

    if not all_gdfs:
        print("❌ No pressure centers found.")
        return
    ...
    if full_gdf.empty:
        print("• No systems inside map boundary.")
        return
    ...
    print(f"Done. Plotted {count} Pressure Centers. ✅")
...
# ==========================================
# --- NEW: GDOT RWIS FETCHER ---
# ==========================================

def compass_to_degrees(compass_str):
    """Converts compass strings (N, NNE, SE) to degrees."""
    if not compass_str or not isinstance(compass_str, str):
        return None
    ...
    return mapping.get(compass_str.upper().strip())

def fetch_gdot_rwis_data():
    """
    Fetches GDOT Road Weather Station data via ArcGIS FeatureServer.
    Maps 'TMPF' (Air) to standard temp and preserves 'ROADT' (Road Temp).
    """
    logging.info("Fetching GDOT RWIS data...")
    url = "https://services1.arcgis.com/2iUE8l8JKrP2tygQ/ArcGIS/rest/services/GDOT_Road_Weather_Stations/FeatureServer/0/query"
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            for feature in data.get('features', []):
                ...
                # Validation: Needs valid coords and Air Temp
                if not geom or not geom.get('coordinates'): continue
                if props.get('TMPF') is None: continue
                
                lon, lat = geom['coordinates']
                
                # Parse Wind (Convert "NW" -> 315)
                wdir_str = props.get('WDIR')
                wdir_deg = compass_to_degrees(wdir_str)
                
                # Parse Dewpoint (Sometimes missing)
                dew = props.get('DPTF')
                
                # Build the station dict compatible with your existing script
                station = {
                    "station_id": props.get('NAME', 'GDOT_UNK'), # e.g. "I85_SR74..."
                    "name": "GDOT Station",
                    "lat": lat,
                    "lon": lon,
                    "temp_f": float(props['TMPF']),  # AIR Temp (for contours)
                    "dew_point_f": float(dew) if dew is not None else None,
                    "wind_dir": wdir_deg,
                    "wind_spd": props.get('WSPD'),   # Assuming Knots based on NWS standard
                    
                    # --- SPECIAL GDOT FIELDS ---
                    "road_temp": props.get('ROADT'),      # The Pavement Temp
                    "road_state": props.get('ROADSTATE'), # "Dry", "Wet", etc.
                    "source": "GDOT"
                }
                stations.append(station)
                
            logging.info(f"Fetched {len(stations)} GDOT stations.")
            return stations
            
    except Exception as e:
        logging.warning(f"Failed to fetch GDOT data: {e}")
        return []

def create_glossy_round_icon(hex_color, size=32):
    """
    Generates a spherical 'Glass Marble' icon.
    FIXED: Cleans corners to prevent square artifacts during zooming.
    """
    # 1. Setup Grid
    ...
    # 3. Circular Mask (Anti-aliased soft edge)
    mask = np.clip(radius - dist_from_center, 0, 1)
    
    # 4. Fill Base (Pre-multiply alpha for clean edges)
    icon[:, :, 0] = r * mask
    icon[:, :, 1] = g * mask
    icon[:, :, 2] = b * mask
    icon[:, :, 3] = mask 

    # 5. "Orb" Shading (Radial Gradient)
    edge_shadow = (dist_from_center / radius) ** 3 
    shadow_intensity = 0.4
    
    for i in range(3):
        # Only darken where mask > 0 to avoid dirtying the transparent corners
        icon[:, :, i] *= (1.0 - (edge_shadow * shadow_intensity))

    # 6. Specular Highlight (The "Glass" Reflection)
    highlight_y = size * 0.25
    highlight_x = size * 0.5
    highlight_dist = np.sqrt(((x - highlight_x)*0.8)**2 + ((y - highlight_y)*1.5)**2)
    glare = np.clip(1.0 - (highlight_dist / (size * 0.4)), 0, 1) ** 3
    
    # Apply White Glare
    for i in range(3):
        icon[:, :, i] = icon[:, :, i] * (1 - glare) + (1.0 * glare * mask)

    # 7. Black Border Ring
    border_mask = np.clip(dist_from_center - (radius - 1.5), 0, 1)
    final_border = border_mask * mask
    
    for i in range(3):
        icon[:, :, i] = icon[:, :, i] * (1 - final_border)
...
def fetch_and_plot_gdot_urban_areas(ax):
    ...
    MATCHED TO FORECAST MAP STYLING:
    Uses a deep charcoal 'footprint' with a specific z-order to ensure 
    visibility on top of hillshaded terrain but below weather data.
    """
    cache_filename = "gdot_urban_areas_production.pkl"
    source_label = "GDOT Urban Areas"
    
    # 1. Check local cache (30-day persistence)
    ...
    if cached_features:
        ...
        params = {'where': '1=1', 'outFields': 'NAME10', 'f': 'geojson', 'outSR': '4326'}
    ...
    # --- VISUAL CONFIGURATION (MATCHED TO FORECAST SCRIPT) ---
    # Color: #111111 (Deep Charcoal)
    # Z-Order: 0.6 
    # This places it exactly above the Terrain (0.5) so it 'tints' the hills
    # but stays below the Satellite IR (1.0) and Temp Contours (1.5).
    face_color = '#111111'
    alpha_val = 0.35
    z_order = 0.6           
    ...
    for feat in features:
        ...
        polys = [coords] if poly_type == 'Polygon' else coords
            
        for poly_coords in polys:
            try:
                # Coordinate cleaning for ArcGIS nesting
                ring = poly_coords[0]
...
def generate_georgia_weather_map():
    """Generate and save the Georgia weather map using Mesonet + RTMA + HRRR + METARs + GDOT."""
    ...
    print("\n" + "="*40)
    print(" 🚀 STARTING MAIN MAP GENERATION ")
    print("="*40)
    logging.info("Generating Georgia weather map (Mesonet + RTMA + HRRR Mode)...")

    # ==========================================
    # 1. CONFIG: NEW ICON PATH & MAPPING
    # ==========================================
    ICON_DIR = "/media/evan/Main/frostbyte/frostbyte_project/icons/new icons"
    
    CONDITION_TO_FILENAME = {
        'Sunny': 'sunny@2x.png',
        'Clear': 'clear@2x.png',
        'Moonlight': 'clearn@2x.png',
        'mostly_sunny': 'fair@2x.png',
        'Mostly Sunny': 'fair@2x.png',
        'Partly Cloudy (Sun)': 'pcloudy@2x.png',
        'Partly Cloudy (Moon)': 'pcloudyn@2x.png',
        'Cloudy': 'cloudy@2x.png',
        'Overcast': 'cloudy@2x.png', 
        'Overcast Night': 'cloudy@2x.png',
        'Rain': 'rain@2x.png',
        'Heavy Rain': 'showers@2x.png', 
        'Storms': 'tstorm@2x.png',
        'Severe Storms': 'tstormn@2x.png',
        'Hail': 'sleet@2x.png', 
        'Fog': 'fog@2x.png',
        'Snow': 'snow@2x.png',
        'Wintry Mix': 'wintrymix@2x.png',
        'Ice Pellets': 'sleet@2x.png',
        'Freezing Rain': 'freezingrain@2x.png',
        'Tornado': 'tstorm@2x.png' 
    }
    ...
    # 2. DATA FETCHING
    # ==========================================
    print("• Fetching Surface Data (IEM/Synoptic)...", end=" ", flush=True)
    iem_results = asyncio.run(get_all_georgia_weather_data_iem())
    print("Done. ✅")
    
    print("• Fetching GDOT RWIS...", end=" ", flush=True)
    gdot_stations = fetch_gdot_rwis_data()
    print("Done. ✅")
    ...
    # A. Process Main "Broadcast" Cities
    ...
    # B. Process "Silent" Stations
    for city_name, (lat, lon, icao) in SILENT_STATIONS.items():
        short_id = icao[1:] if icao.startswith('K') else icao
        station_blob = iem_results.get(icao) or iem_results.get(short_id)
        if station_blob:
            merged = station_blob.copy()
            merged['lat'] = lat
            merged['lon'] = lon
            silent_data_found[icao] = merged
            city_ids_found.add(icao); city_ids_found.add(short_id)

    # C. Process Background Mesonet
    ...
    logging.info(f"Fetched {len(all_station_data)} cities, {len(silent_data_found)} silent, {len(mesonet_stations)} mesonet.")

    # --- FETCH MODEL DATA ---
    print("• Fetching Models (RAP + HRRR + RTMA)...")
    rap_data_full = fetch_rap_full(MAP_EXTENT)
    hrrr_data_sfc = fetch_hrrr_sfc(MAP_EXTENT) 
    rtma_lat, rtma_lon, rtma_temp, rtma_dew, rtma_u, rtma_v, blended_p = blend_rap_hrrr(rap_data_full, hrrr_data_sfc, MAP_EXTENT)
    ...
    # ==========================================
    print("• Sanitizing Data Points...", end=" ", flush=True)
    ...
    gdot_coords = []
    if gdot_stations:
        gdot_coords = np.array([[s['lat'], s['lon']] for s in gdot_stations])
    ...
    if rtma_temp is not None:
        ...
        for data in mesonet_stations:
            ...
            # Duplicate Check (GDOT)
            if len(gdot_coords) > 0:
                dists = np.sum((gdot_coords - np.array([lat, lon]))**2, axis=1)
                if np.min(dists) < 0.000025: continue 

            if sid in SANE_TEMP_EXEMPTION_LIST:
                valid_obs_for_contours.append([lon, lat, t])
                valid_mesonet_for_plot.append(data); continue
            
            model_t = rtma_validator(lon, lat)
            if abs(t - model_t) > DEVIATION_LIMIT: continue
            
            valid_obs_for_contours.append([lon, lat, t])
            valid_mesonet_for_plot.append(data)
            
        # *** MODIFIED: GDOT LOGIC ***
        for data in gdot_stations:
            # We strictly prevent GDOT data from entering 'valid_obs_for_contours'
            # This ensures the pink circles do NOT affect the background color map.
            # We only check if 'road_temp' exists for plotting purposes.
            if data.get('road_temp') is not None:
                valid_gdot_for_plot.append(data)
    else:
        # Fallback if RTMA fails
        for data in mesonet_stations:
            if -20 < data.get('temp_f', -99) < 120:
                valid_obs_for_contours.append([data['lon'], data['lat'], data['temp_f']])
                valid_mesonet_for_plot.append(data)
        
        # *** MODIFIED: GDOT LOGIC (FALLBACK) ***
        for data in gdot_stations:
             # Again, only add to the plot list, NOT the contour list
             if data.get('road_temp') is not None:
                valid_gdot_for_plot.append(data)
    ...
    # ==========================================
    # 2. GRID SETUP & BLENDING (Fixed grid_p)
    # ==========================================
    print("• Calculating Grids (Temp + Pressure)...", end=" ", flush=True)
    grid_z, grid_u, grid_v, grid_x, grid_y, grid_p = None, None, None, None, None, None
    
    if rtma_temp is not None:
        grid_x = rtma_lon; grid_y = rtma_lat; base_grid = rtma_temp 
        # Assign the blended pressure from your model fetch
        grid_p = blended_p 
        ...
        grid_u = gaussian_filter(rtma_u, sigma=0.5)
        grid_v = gaussian_filter(rtma_v, sigma=0.5)
    print("Done. ✅")

    # ==========================================
    # 2.5 PRESSURE GRID (Isobar Optimized - MOVED AFTER AX INIT)
    # ==========================================
    if grid_p is not None:
        print("• Plotting Isobars...", end=" ", flush=True)
        try:
            # Thinned lines (0.7) and lowered alpha (0.6)
            cs_iso_fine = ax.contour(grid_x, grid_y, grid_p, levels=np.arange(900, 1100, 0.5), 
                                     colors="black", linewidths=0.7, alpha=0.6, 
                                     zorder=1.6, transform=ccrs.PlateCarree())
            if clip_patch: cs_iso_fine.set_clip_path(clip_patch)
            
            cs_iso_label = ax.contour(grid_x, grid_y, grid_p, levels=np.arange(900, 1100, 2.0), 
                                      alpha=0, transform=ccrs.PlateCarree())
            clabels_iso = ax.clabel(cs_iso_label, inline=True, fontsize=9, fmt='%1.0f', 
                                    use_clabeltext=True, zorder=1.7)
            
            for label in clabels_iso: 
                label.set_color('white')
                label.set_path_effects([pe.withStroke(linewidth=2.0, foreground='black'), pe.Normal()])
            print("Done. ✅")
        except Exception as e:
             print(f"Error plotting isobars: {e} ❌")

    # ==========================================
    # 3. PLOT INITIALIZATION & BASE GEOGRAPHY
    # ==========================================
    print("\n• Initializing Matplotlib Figure...", end=" ", flush=True)
    ...
    # 3.1 Create the Map Axis (ax)
    map_x, map_y, map_w, map_h = 0.09, 0.12, 0.83, 0.73
    ax = fig.add_axes([map_x, map_y, map_w, map_h], projection=ccrs.PlateCarree())
    ax.set_extent(MAP_EXTENT, crs=ccrs.PlateCarree())

    # 3.2 Setup Clipping Patch
    ...
    # ==========================================
    # 4. COLORMAPS & LAYERS (DEBUGGING HANG)
    # ==========================================
    print("• Setting up Colormaps...", end=" ", flush=True)
    color_stops_data = [
    ...
    if grid_z is not None:
        print("• Plotting Temperature Contours...", end=" ", flush=True)
        try:
            ...
            if clip_patch: contour_plot.set_clip_path(clip_patch)
            ...
            if clip_patch: temp_lines.set_clip_path(clip_patch)
        ...
        print("• Skipping Temp (No Data) ⚪")
    
    # --- PLOT MARINE LAYER (High Visibility Update) ---
    if ncom_lat is not None and grid_z is not None:
        print("• Plotting Marine/Ocean Layer (Optimized)...", end=" ", flush=True)
        try:
            nc_x, nc_y = np.meshgrid(ncom_lon, ncom_lat)
            marine_vort, marine_flux, marine_temp_c = calc_marine_physics(ncom_temp_k, ncom_u, ncom_v, 70, 60, 10)

            # 1. Plot Vorticity contours - Thicker lines
            ax.contour(nc_x, nc_y, marine_vort, levels=[-0.0001, -0.00005, 0.00005, 0.0001], 
                    colors=['blue', 'cyan', 'orange', 'red'], linewidths=1.2, 
                    alpha=0.7, transform=ccrs.PlateCarree(), zorder=1.56)

            # 2. High-Visibility Streamplot
            strm = ax.streamplot(ncom_lon[::2], ncom_lat[::2], ncom_u[::2, ::2], ncom_v[::2, ::2], 
                                color=ncom_sal[::2, ::2], cmap='viridis', 
                                density=1.2,      # Increased for fuller coverage
                                linewidth=1.8,    # Thicker streams
                                arrowsize=1.8,    # Larger arrows
                                transform=ccrs.PlateCarree(), zorder=1.58)
    ...
    if grid_p is not None:
        ...
        try:
            cs_iso_fine = ax.contour(grid_x, grid_y, grid_p, levels=np.arange(900, 1100, 0.5), 
                                     colors="black", linewidths=1.5, alpha=1.0, zorder=1.6, transform=ccrs.PlateCarree())
            if clip_patch: cs_iso_fine.set_clip_path(clip_patch)
            
            cs_iso_label = ax.contour(grid_x, grid_y, grid_p, levels=np.arange(900, 1100, 2.0), alpha=0, transform=ccrs.PlateCarree())
            clabels_iso = ax.clabel(cs_iso_label, inline=True, fontsize=10, fmt='%1.0f hPa', use_clabeltext=True, zorder=1.7)
            
            for label in clabels_iso: 
                label.set_color('white')
                label.set_path_effects([pe.withStroke(linewidth=2.5, foreground='black'), pe.Normal()])
            print("Done. ✅")
        except Exception as e:
             print(f"Error: {e} ❌")
             logging.warning(f"Isobar plotting error: {e}")
    else:
        print("• Skipping Isobars (No Data) ⚪")

    # --- WPC PRESSURE CENTERS ---
    try: 
    ...
    if grid_u is not None:
        ...
        if clip_patch: barb_plot.set_clip_path(clip_patch)
    ...
    # ==========================================
    # 5. PLOT MESONET SQUARES (Border Optimized)
    # ==========================================
    print(f"• Plotting {len(valid_mesonet_for_plot)} sanitized mesonet stations...", end=" ", flush=True)

    glossy_icon_cache = {}
    MESO_ICON_SIZE_PX = 24  
    MESO_ZOOM = 0.30        
    ...
    for station in valid_mesonet_for_plot:
        ...
        # Reduced linewidth to 0.4 for a cleaner look
        ax.scatter(lon, lat, s=120, c="#1900FF", marker='s', 
                   edgecolors='black', linewidth=0.4, 
                   transform=ccrs.PlateCarree(), zorder=9.5)
        ...
        ab = AnnotationBbox(glossy_icon_cache[temp_int], (lon, lat), 
                            frameon=False, pad=0, zorder=10)
        
        if clip_patch: ab.set_clip_path(clip_patch)
        ax.add_artist(ab)
        
        t_label = ax.text(lon, lat, f"{temp_int}", 
                          fontsize=4.5, fontweight='bold', color='white',
                          ha='center', va='center', 
                          transform=ccrs.PlateCarree(), zorder=11, clip_on=True)
        # Thinned out the stroke as well
        t_label.set_path_effects([pe.withStroke(linewidth=1.0, foreground='black'), pe.Normal()])
    print("Done. ✅")

    # ==========================================
    # 5.1 PLOT GDOT CIRCLES (Border Optimized)
    # ==========================================
    print(f"• Plotting {len(valid_gdot_for_plot)} GDOT stations...", end=" ", flush=True)
    
    marble_icon_cache = {}
    MARBLE_SIZE_PX = 24
    MARBLE_ZOOM = 0.30

    for station in valid_gdot_for_plot:
        lat, lon = station['lat'], station['lon']
        temp = station.get('road_temp')
        
        if temp is None: continue
            
        temp_int = int(round(temp))
        
        # Reduced linewidth to 0.4
        ax.scatter(lon, lat, s=120, c="#9C00A1", marker='o', 
                   edgecolors='black', linewidth=0.4, 
                   transform=ccrs.PlateCarree(), zorder=9.5)
        ...
        if temp_int not in marble_icon_cache:
            icon_arr = create_glossy_round_icon(color_hex, size=MARBLE_SIZE_PX)
            marble_icon_cache[temp_int] = OffsetImage(icon_arr, zoom=MARBLE_ZOOM)
            
        ab = AnnotationBbox(marble_icon_cache[temp_int], (lon, lat),
                            frameon=False, pad=0, zorder=10)
        ...
        t_label = ax.text(lon, lat, f"{temp_int}", 
                          ...
                          ha='center', va='center', 
                          transform=ccrs.PlateCarree(), zorder=11, clip_on=True)
        # Thinned out the stroke
        t_label.set_path_effects([pe.withStroke(linewidth=1.0, foreground='black'), pe.Normal()])
    print("Done. ✅")

    # ==========================================
    # 5.5 PLOT SILENT STATIONS
    # ==========================================
    for icao, data in silent_data_found.items():
        lat, lon = data['lat'], data['lon']
        ax.scatter(lon, lat, c='black', s=25, zorder=30, transform=ccrs.PlateCarree(), edgecolors='white', linewidth=0.8)
        text_content = TextArea(icao, textprops=dict(fontsize=7, fontweight='bold', color='black', path_effects=[pe.withStroke(linewidth=2, foreground='white')]))
        ab = AnnotationBbox(text_content, (lon, lat), xybox=(0, -10), xycoords='data', boxcoords="offset points", frameon=False, pad=0.1, zorder=31)
        ax.add_artist(ab)
        _collision_items.append({'item': ab, 'fixed': True})

    # ==========================================
    # 6. MAIN STATION LABELS (Full Ring Visibility Mode)
    # ==========================================
    logging.info("Plotting station text boxes with parameter separation...")
    
    GLOW_CITIES = ["Atlanta", "Savannah", "Augusta", "Columbus", "Macon"]

    def get_text_props(color_hex, fsize=12, is_bold=True):
        """Generates properties with internal padding optimized for ring visibility."""
        return dict(
            color='black', 
            fontsize=fsize, 
            ...
            path_effects=[
                ...
                pe.withStroke(linewidth=2.5, foreground=color_hex), 
                pe.Normal()
            ],
            bbox=dict(
                # pad=0.3 ensures the text doesn't touch the ring
    ...
    props_temp = get_text_props('#FF0000', 15)
    props_dew  = get_text_props('#32CD32', 12)
    props_alt  = get_text_props('#4682B4', 12)
    props_cond = get_text_props('#555555', 12, is_bold=False)
    
    pill_bbox = dict(boxstyle="round,pad=0.3,rounding_size=0.8", facecolor='#555555', edgecolor='none', alpha=0.9)
    props_footer = dict(color='white', fontsize=11, fontweight='bold', 
    ...
    for icao, data in all_station_data.items():
        ...
        box_x, box_y = layout['pos']
        base_lat, base_lon = data["lat"], data["lon"]
        
        # 1. Leader Line
        leader_line, = ax.plot([base_lon, box_x], [base_lat, box_y], color='black', 
                           ls=':', lw=1.5, zorder=25, 
        ...
        if city_name in GLOW_CITIES:
            name_props = dict(
                ...
                path_effects=[
                    pe.SimplePatchShadow(offset=(2, -2), shadow_rgbFace='black', alpha=0.5),
                    pe.withStroke(linewidth=5, foreground="#FF0044", alpha=0.8), 
                    pe.withStroke(linewidth=1.5, foreground='black'), 
                    pe.Normal()
                ],
                bbox=dict(boxstyle="round,pad=0.4,rounding_size=0.2", facecolor='black', 
                          edgecolor='#FF0044', linewidth=2.5, alpha=1.0)
            )
        else:
            name_props = dict(
                color=HEADER_ACCENT, fontsize=13, fontweight='bold',
                path_effects=[
                    pe.SimplePatchShadow(offset=(1.5, -1.5), shadow_rgbFace='black', alpha=0.5),
                    pe.withStroke(linewidth=3, foreground='black'), 
                    pe.Normal()
                ],
                bbox=dict(boxstyle="round,pad=0.4,rounding_size=0.2", facecolor='black', 
                          edgecolor=HEADER_ACCENT, linewidth=1, alpha=1.0)
            )
        ...
        temp_box = TextArea(f" {temp_val:.0f}° " if temp_val is not None else " M ", textprops=props_temp)
        
        dwp_val = data.get("dew_point_f")
        dp_box = TextArea(f" DP {dwp_val:.0f}° " if dwp_val is not None else " DP M ", textprops=props_dew)
        
        alt_val = data.get("alt_inHg")
        tend_arrow = data.get("press_arrow", "")
        alt_box = TextArea(f" {alt_val:.2f} inHg {tend_arrow} " if alt_val is not None else f" M {tend_arrow}", textprops=props_alt)

        cond = data.get("condition", " ")
        cond_box = TextArea(f" {cond.replace('Partially','Ptly').replace('Mostly','Msly')} ", textprops=props_cond)

        # --- PACKING (Fixed Overlap Logic) ---
        # sep=12 in VPacker ensures a significant vertical gap between the box borders
        # align="center" keeps the stacks tidy
        col_left = VPacker(children=[temp_box, cond_box], align="center", pad=2, sep=12)
        col_right = VPacker(children=[dp_box, alt_box], align="center", pad=2, sep=12)
        
        # Combine columns with significant horizontal separation
        grid_packer = HPacker(children=[col_left, col_right], align="center", pad=2, sep=15)
        
        data_assembly = grid_packer
        extras = []
        fl_label, fl_val = calculate_feels_like(temp_val, dwp_val, data.get("wind_spd"))
        if fl_label: extras.append(f"{fl_label} {fl_val}°")
        if data.get("wind_spd") and data.get("wind_spd") >= 5: extras.append(f"W {data['wind_spd']}kt")
        
        if extras:
            footer_box = TextArea(f" {' | '.join(extras)} ", textprops=props_footer)
            # Increased sep=10 here as well to keep the footer from touching the rings
            data_assembly = VPacker(children=[grid_packer, footer_box], align="center", pad=1, sep=10)

        # Final stack with the City Name badge at the top
    ...
    try:
        # padding_px=10 ensures the solver doesn't let badges overlap either
        _resolve_text_collisions(fig, ax, _collision_items, padding_px=10, pixel_step=5)
    except Exception: pass

    # ---------------------------------------------------------
    # MODIFIED ALERT PLOTTING (Lower Z-Order)
    # ---------------------------------------------------------
    alert_handles = {}
    if alerts:
        logging.info(f"Plotting {len(alerts)} active alerts to map...")
        for alert in alerts:
            try:
                event_name = alert.get('event', 'Unknown')
                base_color = alert.get('color', '#808080')
                
                # Create legend handle if new
                if event_name not in alert_handles:
                    alert_handles[event_name] = mpatches.Patch(
                        facecolor=mcolors.to_rgba(base_color, 0.3), 
                ...
                for poly_coords in alert.get('polygons', []):
                    # 1. THE FILL: Semi-transparent background
                    # CHANGED zorder from 80 to 5 (Places it UNDER station icons/barbs)
                    p_fill = mpatches.Polygon(
                    ...
                    # 2. THE BORDER & HATCH: High contrast styling
                    # CHANGED zorder from 81 to 5.01 (Slightly above fill, but under icons)
                    p_hatch = mpatches.Polygon(
                        poly_coords, closed=True, 
                    ...
                    if clip_patch:
                        p_fill.set_clip_path(clip_patch)
                        p_hatch.set_clip_path(clip_patch)
            except Exception as e:
                logging.warning(f"Failed to plot alert polygon: {e}")

    # ---------------------------------------------------------
    # MODIFIED ALERT LEGEND (Multi-Column & Compact)
    # ---------------------------------------------------------
    if alert_handles:
        sorted_alerts = sorted(alert_handles.values(), key=lambda x: x.get_label())
        # Use 2 columns if there are more than 3 alerts to save vertical space
        num_cols = 2 if len(sorted_alerts) > 3 else 1
        
        leg_alerts = ax.legend(
            handles=sorted_alerts, loc='upper left', bbox_to_anchor=(0.0, 1.0),
            title="Active Alerts", fontsize=7.5, facecolor='white', framealpha=0.85, 
            edgecolor='red', fancybox=True, ncol=num_cols,
        ...
        leg_alerts.get_title().set_fontsize(9)
        
        for t in leg_alerts.get_texts(): 
            t.set_color("black")
            t.set_fontweight("bold")
        ax.add_artist(leg_alerts)

    # --- MAIN STATIC LEGEND (Top Right) ---
    legend_handles = [
        mlines.Line2D([], [], color='black', marker='*', ls='None', ms=9, mec='darkblue', label='City Dot'),
        mlines.Line2D([], [], color='#FFD700', marker='+', ls='None', ms=10, mew=2, label='GLM Lightning'),
        
        mlines.Line2D([], [], color='white', marker='s', mfc='white', mec='red', mew=2.0, ms=8, ls='None', label='Temperature (Red Box)'),
        mlines.Line2D([], [], color='white', marker='o', mfc='white', mec="#FF00C8", mew=2.0, ms=8, ls='None', label='Road Temp (Pink Ring)'),
        
        mlines.Line2D([], [], color='white', marker='s', mfc='white', mec='green', mew=1.5, ms=8, ls='None', label='Dew Pt / Feels Like'),
        mlines.Line2D([], [], color='white', marker='s', mfc='white', mec='#4682B4', mew=1.5, ms=8, ls='None', label='Pressure / Wind'),
        ...
        mlines.Line2D([], [], color='blue', lw=2.5, label='Cold Front', path_effects=[ColdFront(size=6, spacing=1.0)]),
        mlines.Line2D([], [], color='red', lw=2.5, label='Warm Front', path_effects=[WarmFront(size=6, spacing=1.0)]),
        mlines.Line2D([], [], color='purple', lw=2.5, label='Occluded Front', path_effects=[OccludedFront(size=6, spacing=1.0)]),
    ]
    ...
    leg = ax.legend(handles=legend_handles, loc='upper right', fontsize=12,
                    bbox_to_anchor=(1.0, 1.0), facecolor='white', framealpha=0.9, 
                    edgecolor=HEADER_ACCENT, fancybox=True, ncol=2,
                    handletextpad=0.5, borderpad=0.4, columnspacing=1.0)
    leg.set_zorder(100)
    
    main_leg_stroke = [pe.withStroke(linewidth=2.5, foreground='black'), pe.Normal()]
    
    for t in leg.get_texts(): 
        t.set_color(HEADER_ACCENT) 
        t.set_fontweight('bold')   
        t.set_path_effects(main_leg_stroke) 

    # --- FLIGHT CATEGORY LEGEND (Bottom Left) ---
    arrow_icon = mlines.Line2D([], [], color='black', marker='>', 
                               linestyle='None', markersize=12, 
                               mec='black', mfc='black', label='Wind Direction')

    def make_circle_handle(color, label):
        return mlines.Line2D([], [], color=color, marker='o', linestyle='None',
                             markersize=12, mec='black', mfc=color, label=label)

    flight_handles = [
        make_circle_handle(FLIGHT_CATEGORY_COLORS["VFR"], 'VFR (Clear)'),
        make_circle_handle(FLIGHT_CATEGORY_COLORS["MVFR"], 'MVFR (Marginal)'),
        make_circle_handle(FLIGHT_CATEGORY_COLORS["IFR"], 'IFR (Low Clouds/Vis)'),
        make_circle_handle(FLIGHT_CATEGORY_COLORS["LIFR"], 'LIFR (Very Low)'),
        arrow_icon 
    ]

    leg_flight = ax.legend(handles=flight_handles, loc='lower left', fontsize=10, 
                           title="Station Flight Rules",
                           bbox_to_anchor=(0.02, 0.02), facecolor='white', 
                           framealpha=0.9, edgecolor=HEADER_ACCENT, fancybox=True)
    leg_flight.set_zorder(100)

    title = leg_flight.get_title()
    title.set_color(HEADER_ACCENT) 
    title.set_fontweight('bold')
    title.set_path_effects([pe.withStroke(linewidth=2.5, foreground='black'), pe.Normal()])

    thin_black_stroke = [pe.withStroke(linewidth=1.0, foreground='black'), pe.Normal()]
    
    for text in leg_flight.get_texts():
        text.set_color(HEADER_ACCENT) 
        text.set_fontweight('normal') 
        text.set_path_effects(thin_black_stroke)
    ...
    buf = io.BytesIO()
    print("• Saving Final Image to Disk...", end=" ", flush=True)
    plt.savefig(buf, format='png', facecolor=fig.get_facecolor(), dpi=300)
    buf.seek(0)
    
    timestamp = datetime.now().strftime('%Y-%m-%d_%H%M%S')
    with open(os.path.join(OUTPUT_DIR, f"ga_hrrr_weather_{timestamp}.png"), 'wb') as f: 
        f.write(buf.getbuffer())
    buf.seek(0)
    with open(os.path.join(PROJECT_ROOT, "ga_detailed_weather.png"), 'wb') as f: 
        f.write(buf.getbuffer())

    # ==========================================
    # 8. STATUS DASHBOARD SUMMARY
    # ==========================================
    print("\n" + "═"*50)
    print(" 📊 SCRIPT PERFORMANCE & CACHE DASHBOARD ")
    print("═"*50)
    
    total_requests = STATUS_TRACKER["Cache Hits"] + STATUS_TRACKER["Cache Misses"]
    hit_ratio = (STATUS_TRACKER["Cache Hits"] / total_requests * 100) if total_requests > 0 else 0
    
    print(f"• Total Data Requests:  {total_requests}")
    print(f"• Cache Hit Ratio:      {hit_ratio:.1f}%")
    print(f"• Live Downloads:       {STATUS_TRACKER['Downloads']['Successful']} (Success) / {STATUS_TRACKER['Downloads']['Failed']} (Failed)")
    print("-" * 30)
    
    for source, stats in STATUS_TRACKER["Data Sources"].items():
        status_icon = "✅" if stats["Hits"] > 0 else "🌐"
        print(f"{status_icon} {source:20} | Hits: {stats['Hits']} | Misses: {stats['Misses']}")

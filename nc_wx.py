...
# -*- coding: utf-8 -*-
"""
Standalone North Carolina (NC) Weather Map Generator.
...
NEXRAD radar image to generate a detailed weather map for the state of North Carolina.

The final map is saved as 'nc_detailed_weather.png'.
"""
...
# --- Configuration (NORTH CAROLINA) ---
MAP_EXTENT = [-85.0, -75.0, 33.5, 37.0] # [lon_min, lon_max, lat_min, lat_max]
CITIES = {
    "Charlotte": (35.2140, -80.9431, "KCLT"),
    "Raleigh": (35.8776, -78.7875, "KRDU"),
    "Greensboro": (36.0978, -79.9373, "KGSO"),
    "Winston-Salem": (36.1337, -80.2220, "KINT"),
    "Asheville": (35.4362, -82.5418, "KAVL"),
    "Wilmington": (34.2706, -77.9026, "KILM"),
    "Fayetteville": (34.9912, -78.8803, "KFAY"),
    "Hickory": (35.7422, -81.3903, "KHKY"),
    "Jacksonville": (34.8291, -77.6122, "KOAJ"),
    "New Bern": (35.0729, -77.0429, "KEWN"),
    "Greenville": (35.6358, -77.3853, "KPGV"),
    "Rocky Mount": (35.8569, -77.8931, "KRWI"),
    "Jefferson": (36.4132, -81.4437, "KGEV"), # For Boone area
    "Manteo": (35.9184, -75.7001, "KMQI"),    # Outer Banks
    "Hatteras": (35.2325, -75.6175, "KHSE"),  # Outer Banks
    "Elizabeth City": (36.2603, -76.1739, "KECG"),
}
CITY_OFFSETS = {
    # Triangle Cluster
    "Raleigh": (0.1, 0.1),
    "Fayetteville": (0.1, -0.1), # Move SE
    
    # Triad Cluster
    "Greensboro": (-0.15, 0.1), # Move NW
    "Winston-Salem": (0.15, -0.1), # Move SE
    
    # Charlotte Cluster
    "Charlotte": (0.1, -0.1), # Move SE
    "Hickory": (-0.1, 0.1), # Move NW
    
    # Mountain Cluster
    "Asheville": (0.1, -0.1), # Move SE
    "Jefferson": (0.1, 0.1), # Move NE
}
SILENT_STATIONS = {
    # In-state fill-ins
    "Statesville": (35.7650, -80.9539, "KSVH"),
    "Salisbury": (35.6446, -80.5208, "KRUQ"),
    "N. Wilkesboro": (36.1478, -81.0770, "KUKF"),
    "Johnston Co.": (35.5303, -78.4878, "KJNX"),
    "Lumberton": (34.6158, -79.0594, "KLBT"),
    "Shelby": (35.2539, -81.6031, "KEHO"),
    "Franklin": (35.1189, -83.4078, "K1A5"),
    
    # Border stations for "pinning" the edges
    "Danville (VA)": (36.5722, -79.3361, "KDAN"),
    "Roanoke (VA)": (37.3255, -79.9754, "KROA"),
    "Norfolk (VA)": (36.8946, -76.2012, "KORF"),
    "Tri-Cities (TN)": (36.4752, -82.4074, "KTRI"),
    "Knoxville (TN)": (35.8109, -83.9940, "KTYS"),
    "Spartanburg (SC)": (34.8956, -82.2188, "KGSP"),
    "Myrtle Beach (SC)": (33.6797, -78.9283, "KMYR"),
    "Florence (SC)": (34.1853, -79.7239, "KFLO"),
    "Toccoa (GA)": (34.5928, -83.2961, "KTOC"),
}
# ---
FIG_BG_COLOR = '#b0c4de'
LAND_COLOR = '#f5f5dc'
...
def get_nws_alerts(state='NC'):
    """Fetches active NWS alerts for a given state (e.g., 'NC')."""
...
def generate_north_carolina_weather_map(): # <-- Renamed function
    """Generate and save the North Carolina weather map."""
    
    logging.info("Generating North Carolina weather map...")
    
    # --- Combine CITIES and SILENT_STATIONS ---
    ALL_STATIONS_NC = CITIES.copy()
    ALL_STATIONS_NC.update(SILENT_STATIONS)
    ...
    metar_cache_file = "metar_nc_cache.json"
    ...
    if metar_results is None:
        ...
        metar_results = asyncio.run(fetch_all_metars_async(ALL_STATIONS_NC))
    ...
    alert_cache_file = "alerts_nc_cache.json"
    alerts = load_json_cache(alert_cache_file, CACHE_DURATION_SECONDS)
    
    if alerts is None:
        logging.info("Fetching NWS alerts for North Carolina...")
        alerts = get_nws_alerts('NC') # <-- Updated for NC
    ...
    for city, temp in all_temperatures.items():
        lat, lon, _ = ALL_STATIONS_NC[city]
    ...
    fig.suptitle(f"North Carolina - Detailed Weather Map\nGenerated: {current_time}",
    ...
    clip_patch = None
    try:
        logging.info("Creating state clipping path for North Carolina...")
        target_states = ['North Carolina'] # <-- Updated for NC
        state_geometries = []
        ...
        if not state_geometries:
            raise ValueError("Could not find North Carolina in shpreader.")
        combined_geom = unary_union(state_geometries)
        if not combined_geom:
            raise ValueError("Unary union of North Carolina geometry failed.")
        paths = geos_to_path(combined_geom)
        if not paths:
            raise ValueError("geos_to_path returned no paths for North Carolina.")
        ...
        if clip_patch:
            ...
            raise ValueError("PathPatch creation failed for North Carolina.")
    except Exception as e:
        logging.error(f"Failed to create clip patch for North Carolina. Error: {e}")
        clip_patch = None
    if not clip_patch:
    ...
    for city, temp in all_temperatures.items():
        base_lat, base_lon, icao = ALL_STATIONS_NC[city]
    ...
    wwa_text = "### 🔔 Active Weather Alerts in North Carolina:\n" # <-- Updated for NC
    if alerts:
        event_groups = {}
        for alert in alerts:
            evt = alert['event']
        ...
        wwa_text = "✅ No active weather alerts for North Carolina at this time." # <-- Updated for NC
...
# --- *** NEW DUAL-SAVE LOGIC *** ---
    ...
    dynamic_filename = f"nc_detailed_weather_{timestamp_str}.png"
    static_filename = "nc_detailed_weather.png" # For the GUI preview
...
if __name__ == "__main__":
    ...
    try:
        generate_north_carolina_weather_map() # <-- Renamed function

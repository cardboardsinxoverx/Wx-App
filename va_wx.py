...
# -*- coding: utf-8 -*-
"""
Standalone Virginia (VA) Weather Map Generator.
...
NEXRAD radar image to generate a detailed weather map for the state of Virginia.
...
# --- Configuration (VIRGINIA) ---
MAP_EXTENT = [-84.0, -75.0, 36.3, 39.7] # [lon_min, lon_max, lat_min, lat_max]
CITIES = {
    "Richmond": (37.5407, -77.4360, "KRIC"),
    "Norfolk": (36.8529, -76.2859, "KORF"),
    "Roanoke": (37.2638, -79.9414, "KROA"),
    "Lynchburg": (37.4138, -79.1422, "KLYH"),
    "Charlottesville": (38.0393, -78.4767, "KCHO"),
    "Dulles (IAD)": (38.9445, -77.4558, "KIAD"),
    "Arlington (DCA)": (38.8521, -77.0377, "KDCA"),
    "Blacksburg": (37.2296, -80.4139, "KBCB"),
    "Danville": (36.5776, -79.3953, "KDAN"),
    "Newport News": (37.0829, -76.4228, "KPHF"),
    "Wallops Island": (37.8360, -75.4830, "KWAL"),
    "Harrisonburg": (38.2638, -78.8975, "KSHD"),
}
CITY_OFFSETS = {
    # Guesses: Tune these if labels overlap
    "Arlington (DCA)": (0.2, 0.05),
    "Dulles (IAD)": (-0.1, -0.1),
    "Norfolk": (0.1, -0.1),
    "Newport News": (-0.1, 0.1),
}
SILENT_STATIONS = {
    # In-state fill-ins
    "Farmville": (37.3038, -78.3947, "KFVX"),
    "Martinsville": (36.6876, -79.8653, "KMTV"),
    "Pulaski": (37.0423, -80.6791, "KPSK"),
    "Tangier": (37.8268, -75.9866, "KTGI"),
    
    # Border stations for "pinning" the edges
    "Tri-Cities (TN)": (36.4752, -82.4074, "KTRI"),
    "Raleigh-Durham (NC)": (35.8776, -78.7875, "KRDU"),
    "Greensboro (NC)": (36.0978, -79.9373, "KGSO"),
    "Lewisburg (WV)": (37.8592, -80.3992, "KLWB"),
    "Charleston (WV)": (38.3731, -81.5931, "KCRW"),
    "Morgantown (WV)": (39.6428, -79.9161, "KMGW"),
    "Hagerstown (MD)": (39.7092, -77.7289, "KHGR"),
    "Salisbury (MD)": (38.3401, -75.5103, "KSBY"),
    "Ocean City (MD)": (38.3109, -75.2255, "KOXB"),
}
# ---
FIG_BG_COLOR = '#b0c4de'
LAND_COLOR = '#f5f5dc'
...
def get_nws_alerts(state='VA'):
    """Fetches active NWS alerts for a given state (e.g., 'VA')."""
...
def generate_virginia_weather_map(): # <-- No longer async
    """Generate and save the Virginia weather map."""
    
    logging.info("Generating Virginia weather map...")
    
    # --- Combine CITIES and SILENT_STATIONS ---
    ALL_STATIONS_VA = CITIES.copy()
    ALL_STATIONS_VA.update(SILENT_STATIONS)
    ...
    metar_cache_file = "metar_va_cache.json"
    ...
    if metar_results is None:
        ...
        metar_results = asyncio.run(fetch_all_metars_async(ALL_STATIONS_VA))
    ...
    alert_cache_file = "alerts_va_cache.json"
    alerts = load_json_cache(alert_cache_file, CACHE_DURATION_SECONDS)
    
    if alerts is None:
        logging.info("Fetching NWS alerts for Virginia...")
        alerts = get_nws_alerts('VA')
    ...
    for city, temp in all_temperatures.items():
        lat, lon, _ = ALL_STATIONS_VA[city]
    ...
    fig.suptitle(f"Virginia - Detailed Weather Map\nGenerated: {current_time}",
    ...
    clip_patch = None
    try:
        logging.info("Creating state clipping path for Virginia...")
        target_states = ['Virginia']
        state_geometries = []
        ...
        if not state_geometries:
            raise ValueError("Could not find Virginia in shpreader.")
        combined_geom = unary_union(state_geometries)
        if not combined_geom:
            raise ValueError("Unary union of Virginia geometry failed.")
        paths = geos_to_path(combined_geom)
        if not paths:
            raise ValueError("geos_to_path returned no paths for Virginia.")
        ...
        if clip_patch:
            ...
            raise ValueError("PathPatch creation failed for Virginia.")
    except Exception as e:
        logging.error(f"Failed to create clip patch for Virginia. Error: {e}")
        clip_patch = None
    if not clip_patch:
    ...
    for city, temp in all_temperatures.items():
        base_lat, base_lon, icao = ALL_STATIONS_VA[city]
    ...
    wwa_text = "### 🔔 Active Weather Alerts in Virginia:\n"
    if alerts:
        event_groups = {}
        for alert in alerts:
            evt = alert['event']
        ...
        wwa_text = "✅ No active weather alerts for Virginia at this time."
...
# --- *** NEW DUAL-SAVE LOGIC *** ---
    ...
    dynamic_filename = f"va_detailed_weather_{timestamp_str}.png"
    static_filename = "va_detailed_weather.png" # For the GUI preview

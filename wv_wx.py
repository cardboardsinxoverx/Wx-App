...
# -*- coding: utf-8 -*-
"""
Standalone West Virginia (WV) Weather Map Generator.
...
NEXRAD radar image to generate a detailed weather map for the state of West Virginia.

The final map is saved as 'wv_detailed_weather.png'.
"""
...
# --- Configuration (WEST VIRGINIA) ---
MAP_EXTENT = [-83.0, -77.5, 37.0, 40.8] # [lon_min, lon_max, lat_min, lat_max]
CITIES = {
    "Charleston": (38.3731, -81.5931, "KCRW"),
    "Huntington": (38.3653, -82.5550, "KHTS"),
    "Morgantown": (39.6428, -79.9161, "KMGW"),
    "Clarksburg": (39.2970, -80.2281, "KCKB"),
    "Beckley": (37.7881, -81.1231, "KBKW"),
    "Lewisburg": (37.8592, -80.3992, "KLWB"),
    "Wheeling": (40.1747, -80.6450, "KHLG"),
    "Martinsburg": (39.4019, -77.9850, "KMRB"),
    "Elkins": (38.8897, -79.8572, "KEKN"),
    "Parkersburg": (39.3450, -81.4386, "KPKB"),
}
CITY_OFFSETS = {
    # No obvious overlaps, can be tuned later
}
SILENT_STATIONS = {
    # In-state fill-ins
    "Summersville": (38.2312, -80.8645, "KSXL"),
    "Bluefield": (37.2958, -81.2064, "KBLF"),
    
    # Border stations for "pinning" the edges
    "Athens (OH)": (39.2292, -82.2319, "KUNI"),
    "Pikeville (KY)": (37.5622, -82.5706, "KPBX"),
    "Roanoke (VA)": (37.3181, -79.9733, "KROA"),
    "Harrisonburg (VA)": (38.2638, -78.8975, "KSHD"),
    "Cumberland (MD)": (39.6156, -78.7614, "KCBE"),
    "Hagerstown (MD)": (39.7092, -77.7289, "KHGR"),
    "Johnstown (PA)": (40.3161, -78.8339, "KJST"),
    "Pittsburgh (PA)": (40.4914, -80.2328, "KPIT"),
}
# ---
FIG_BG_COLOR = '#b0c4de'
LAND_COLOR = '#f5f5dc'
...
def get_nws_alerts(state='WV'):
    """Fetches active NWS alerts for a given state (e.g., 'WV')."""
...
def generate_west_virginia_weather_map(): # <-- No longer async
    """Generate and save the West Virginia weather map."""
    
    logging.info("Generating West Virginia weather map...")
    
    # --- Combine CITIES and SILENT_STATIONS ---
    ALL_STATIONS_WV = CITIES.copy()
    ALL_STATIONS_WV.update(SILENT_STATIONS)
    ...
    metar_cache_file = "metar_wv_cache.json"
    ...
    if metar_results is None:
        ...
        metar_results = asyncio.run(fetch_all_metars_async(ALL_STATIONS_WV))
    ...
    alert_cache_file = "alerts_wv_cache.json"
    alerts = load_json_cache(alert_cache_file, CACHE_DURATION_SECONDS)
    
    if alerts is None:
        logging.info("Fetching NWS alerts for West Virginia...")
        alerts = get_nws_alerts('WV')
    ...
    for city, temp in all_temperatures.items():
        lat, lon, _ = ALL_STATIONS_WV[city]
    ...
    fig.suptitle(f"West Virginia - Detailed Weather Map\nGenerated: {current_time}",
    ...
    clip_patch = None
    try:
        logging.info("Creating state clipping path for West Virginia...")
        target_states = ['West Virginia']
        state_geometries = []
        ...
        if not state_geometries:
            raise ValueError("Could not find West Virginia in shpreader.")
        combined_geom = unary_union(state_geometries)
        if not combined_geom:
            raise ValueError("Unary union of West Virginia geometry failed.")
        paths = geos_to_path(combined_geom)
        if not paths:
            raise ValueError("geos_to_path returned no paths for West Virginia.")
        ...
        if clip_patch:
            ...
            raise ValueError("PathPatch creation failed for West Virginia.")
    except Exception as e:
        logging.error(f"Failed to create clip patch for West Virginia. Error: {e}")
        clip_patch = None
    if not clip_patch:
    ...
    for city, temp in all_temperatures.items():
        base_lat, base_lon, icao = ALL_STATIONS_WV[city]
    ...
    wwa_text = "### 🔔 Active Weather Alerts in West Virginia:\n"
    if alerts:
        event_groups = {}
        for alert in alerts:
            evt = alert['event']
        ...
        wwa_text = "✅ No active weather alerts for West Virginia at this time."
...
# --- *** NEW DUAL-SAVE LOGIC *** ---
    ...
    dynamic_filename = f"wv_detailed_weather_{timestamp_str}.png"
    static_filename = "wv_detailed_weather.png" # For the GUI preview
...
if __name__ == "__main__":
    ...
    try:
        generate_west_virginia_weather_map()

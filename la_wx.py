...
# -*- coding: utf-8 -*-
"""
Standalone Louisiana (LA) Weather Map Generator.
...
NEXRAD radar image to generate a detailed weather map for the state of Louisiana.

The final map is saved as 'la_detailed_weather.png'.
"""
...
# --- Configuration (LOUISIANA) ---
MAP_EXTENT = [-94.2, -88.7, 28.8, 33.2] # [lon_min, lon_max, lat_min, lat_max]
CITIES = {
    "New Orleans": (29.9934, -90.2592, "KMSY"),
    "Baton Rouge": (30.5332, -91.1496, "KBTR"),
    "Shreveport": (32.4665, -93.8256, "KSHV"),
    "Lafayette": (30.2056, -91.9875, "KLFT"),
    "Lake Charles": (30.1261, -93.2230, "KLCH"),
    "Monroe": (32.5108, -92.0375, "KMLU"),
    "Alexandria": (31.3274, -92.5497, "KAEX"),
}
CITY_OFFSETS = {
    "New Orleans": (0.1, -0.1),
    "Baton Rouge": (0.1, 0.1),
...
SILENT_STATIONS = {
    ...
    "Texarkana (AR/TX)": (33.4536, -94.0186, "KTXK"),
    "Lufkin (TX)": (31.2828, -94.7508, "KLFK"),
    "Beaumont (TX)": (29.9508, -94.0208, "KBPT"),
    "Jackson (MS)": (32.3112, -90.0759, "KJAN"),
    "McComb (MS)": (31.2292, -90.4725, "KMCB"),
    "Mobile (AL)": (30.6869, -88.2428, "KMOB"), # Still useful for SE corner
    "Natchez (MS)": (31.612, -91.296, "KHEZ"),
}
# ---
FIG_BG_COLOR = '#b0c4de'
LAND_COLOR = '#f5f5dc'
...
def get_nws_alerts(state='LA'):
    """Fetches active NWS alerts for a given state (e.g., 'LA')."""
...
def generate_louisiana_weather_map(): # <-- [CHANGED] No longer async
    """Generate and save the Louisiana weather map."""
    
    logging.info("Generating Louisiana weather map...")
    
    ALL_STATIONS_LA = CITIES.copy()
    ALL_STATIONS_LA.update(SILENT_STATIONS)
    ...
    metar_cache_file = "metar_la_cache.json"
    ...
    if metar_results is None:
        ...
        metar_results = asyncio.run(fetch_all_metars_async(ALL_STATIONS_LA))
    ...
    alert_cache_file = "alerts_la_cache.json"
    alerts = load_json_cache(alert_cache_file, CACHE_DURATION_SECONDS)
    
    if alerts is None:
        logging.info("Fetching NWS alerts for Louisiana...")
        alerts = get_nws_alerts('LA')
    ...
    for city, temp in all_temperatures.items():
        lat, lon, _ = ALL_STATIONS_LA[city]
    ...
    fig.suptitle(f"Louisiana - Detailed Weather Map\nGenerated: {current_time}",
    ...
    clip_patch = None
    try:
        logging.info("Creating state clipping path for Louisiana...")
        target_states = ['Louisiana']
        state_geometries = []
        # --- Use 50m for speed ---
        ...
        if not state_geometries:
            raise ValueError("Could not find Louisiana in shpreader.")
        combined_geom = unary_union(state_geometries)
        if not combined_geom:
            raise ValueError("Unary union of Louisiana geometry failed.")
        paths = geos_to_path(combined_geom)
        if not paths:
            raise ValueError("geos_to_path returned no paths for Louisiana.")
        ...
        if clip_patch:
            ...
            raise ValueError("PathPatch creation failed for Louisiana.")
    except Exception as e:
        logging.error(f"Failed to create clip patch for Louisiana. Error: {e}")
        clip_patch = None
    if not clip_patch:
    ...
    try:
        ...
        # Note: USCOUNTIES calls counties "parishes" in Louisiana, so this will work
        ...
        logging.warning(f"Error loading counties (parishes) with MetPy: {e}")
    ...
    for city, temp in all_temperatures.items():
        base_lat, base_lon, icao = ALL_STATIONS_LA[city]
    ...
    legend_handles = [
        ...
        mlines.Line2D([], [], color=COUNTY_BORDER_COLOR, linestyle='-.', linewidth=0.8, label='Parish Border'),
    ...
    wwa_text = "### 🔔 Active Weather Alerts in Louisiana:\n"
    if alerts:
        event_groups = {}
        for alert in alerts:
            evt = alert['event']
        ...
        wwa_text = "✅ No active weather alerts for Louisiana at this time."
...
# --- *** NEW DUAL-SAVE LOGIC *** ---
    ...
    dynamic_filename = f"la_detailed_weather_{timestamp_str}.png"
    static_filename = "la_detailed_weather.png" # For the GUI preview
...
if __name__ == "__main__":
    ...
    """
    try:
        generate_louisiana_weather_map() # <-- [CHANGED]

...
# -*- coding: utf-8 -*-
"""
Standalone Texas (TX) Weather Map Generator.
...
NEXRAD radar image to generate a detailed weather map for the state of Texas.

The final map is saved as 'tx_detailed_weather.png'.
"""
...
# --- Configuration (TEXAS) ---
MAP_EXTENT = [-106.8, -93.2, 25.7, 36.7] # [lon_min, lon_max, lat_min, lat_max]
CITIES = {
    "Abilene": (32.4113, -99.6818, "KABI"),
    "Amarillo": (35.2219, -101.8333, "KAMA"),
    "Austin": (30.2672, -97.7431, "KAUS"),
    "Brownsville": (25.9069, -97.4194, "KBRO"),
    "Corpus Christi": (27.7719, -97.5003, "KCRP"),
    "Dallas": (32.7767, -96.7970, "KDAL"),
    "El Paso": (31.7619, -106.4850, "KELP"),
    "Fort Worth": (32.7555, -97.3208, "KAFW"),
    "Houston": (29.7604, -95.3698, "KIAH"),
    "Laredo": (27.5436, -99.4615, "KLRD"),
    "Lubbock": (33.6637, -101.8422, "KLBB"),
    "Midland": (31.9425, -102.1938, "KMAF"),
    "San Antonio": (29.4241, -98.4936, "KSAT"),
    "Tyler": (32.3543, -95.3025, "KTYR"),
    "Wichita Falls": (33.9137, -98.4891, "KSPS"),
}
CITY_OFFSETS = {
    "Houston": (0.1, -0.1),
    "Dallas": (0.25, 0.15),
    "Fort Worth": (-0.25, 0.15),
    "Austin": (0.1, 0.1),
    "San Antonio": (-0.1, -0.1),
    "El Paso": (0.25, 0.05),
    "Wichita Falls": (0.1, 0.1),
    "Brownsville": (0.0, 0.15),
}
# --- NEW: Added empty SILENT_STATIONS to match ga_wx.py structure ---
SILENT_STATIONS = {
    # e.g., "Childress": (34.425, -100.290, "KCDS"),
    # e.g., "Shreveport (LA)": (32.466, -93.825, "KSHV"),
}
# ---
FIG_BG_COLOR = '#b0c4de'
LAND_COLOR = '#f5f5dc'
...
# --- Helper Functions (Copied from GA_WX) ---
...
def get_nws_alerts(state='TX'):
    """Fetches active NWS alerts for a given state (e.g., 'TX')."""
...
def generate_texas_weather_map(): # <-- CHANGED: No longer async
    """Generate and save the Texas weather map."""
    
    logging.info("Generating Texas weather map...")
    
    # --- NEW: Combine CITIES and SILENT_STATIONS ---
    ALL_STATIONS_TX = CITIES.copy()
    ALL_STATIONS_TX.update(SILENT_STATIONS)
    ...
    metar_cache_file = "metar_tx_cache.json"
    ...
    if metar_results is None:
        ...
        metar_results = asyncio.run(fetch_all_metars_async(ALL_STATIONS_TX))
    ...
    alert_cache_file = "alerts_tx_cache.json"
    alerts = load_json_cache(alert_cache_file, CACHE_DURATION_SECONDS)
    
    if alerts is None:
        logging.info("Fetching NWS alerts for Texas...")
        alerts = get_nws_alerts('TX')
    ...
    # --- MODIFIED: Use ALL_STATIONS_TX for interpolation ---
    for city, temp in all_temperatures.items():
        lat, lon, _ = ALL_STATIONS_TX[city]
    ...
    if len(temp_values) >= 4:
        logging.info("Interpolating gridded data...")
        try:
            # Use the defined map extent for the grid
            grid_lon = np.linspace(MAP_EXTENT[0], MAP_EXTENT[1], 150)
            grid_lat = np.linspace(MAP_EXTENT[2], MAP_EXTENT[3], 150)
    ...
    fig = plt.figure(figsize=(25, 20)) # Texas is wide, keep this size
    fig.patch.set_facecolor(FIG_BG_COLOR)
    
    text_outline = [pe.withStroke(linewidth=3, foreground='white')]
    
    current_time = datetime.now().strftime('%Y-%m-%d %I:%M %p') # Use %I:%M %p for 12-hr
    fig.suptitle(f"Texas - Detailed Weather Map\nGenerated: {current_time}",
    ...
    # --- NEW: Robust STATE CLIP PATCH (from GA_WX) ---
    clip_patch = None
    try:
        logging.info("Creating state clipping path for Texas...")
        target_states = ['Texas'] # <-- Changed to Texas
        state_geometries = []
        ...
        if not state_geometries:
            raise ValueError("Could not find Texas in shpreader.")
        combined_geom = unary_union(state_geometries)
        if not combined_geom:
            raise ValueError("Unary union of Texas geometry failed.")
        paths = geos_to_path(combined_geom)
        if not paths:
            raise ValueError("geos_to_path returned no paths for Texas.")
        ...
        if clip_patch:
            ...
            raise ValueError("PathPatch creation failed for Texas.")
    except Exception as e:
        logging.error(f"Failed to create clip patch for Texas. Error: {e}")
        clip_patch = None
    if not clip_patch:
    ...
    if grid_u is not None and grid_v is not None and grid_lon is not None and grid_lat is not None:
        s = 10 # Keep Texas's sparser grid
    ...
    try:
        counties_feature = USCOUNTIES.with_scale('5m') # Use 5m for better detail on large map
    ...
    try:
        # Use 50m roads for speed, 10m is too slow for all of TX
    ...
    gl.xlabel_style = {'size': 9, 'path_effects': text_outline}
    gl.ylabel_style = {'size': 9, 'path_effects': text_outline}
    ...
    # --- NEW: Station plotting loop (from GA_WX) ---
    # This loop includes the if/else logic for CITIES vs SILENT_STATIONS
    logging.info("Plotting station data...")
    for city, temp in all_temperatures.items():
        base_lat, base_lon, icao = ALL_STATIONS_TX[city]
        ...
        if city in CITIES:
            ...
            if dew_point is not None:
                dp_text = ax.text(plot_lon - 0.07, plot_lat - 0.13, f"DP: {dew_point:.0f}°F", color='#228b22', # y-offset adjusted for font
                        bbox=BBOX_STYLE, transform=ccrs.PlateCarree(), fontsize=FONT_SIZE_DATA,
            ...
            if alt_inHg is not None:
                alt_text = ax.text(plot_lon + 0.07, plot_lat - 0.13, f"{alt_inHg:.2f}\"Hg", color='#4169e1', # y-offset adjusted for font
                        bbox=BBOX_STYLE, transform=ccrs.PlateCarree(), fontsize=FONT_SIZE_DATA,
            ...
            if condition:
                ...
                if condition in WEATHER_ICONS:
                    try:
                        ...
                        im = OffsetImage(img, zoom=0.2) # TX font is smaller, so keep zoom 0.2
                        text_area = TextArea(
                            f" {cond_text}",
                            textprops=dict(
                                color='darkred',
                        ...
                        ab = AnnotationBbox(
                            packer,
                            (plot_lon, plot_lat - 0.23), # y-offset adjusted for font
                            xybox=(0, 0),
                            xycoords='data',
                        ...
                        cond_text_obj = ax.text(plot_lon, plot_lat - 0.23, cond_text, color='darkred',
    ...
    # --- NEW: Main Temp Colorbar (from GA_WX) ---
    # This is placed *outside* the map axes and does not shrink it.
    divider = make_axes_locatable(ax)
    ...
    legend_handles = [
        ...
        mpatches.Patch(color=SECONDARY_ROAD_COLOR, label='Secondary Highway', alpha=0.7), # Added secondary
    ]
    legend_handles.extend(alert_handles.values())

    # --- MODIFIED: Legend placed in bottom-right ---
    leg = ax.legend(handles=legend_handles, loc='lower right', fontsize=FONT_SIZE_LEGEND,
                    ...
                    edgecolor='lightgray', fancybox=True, ncol=2) # Added ncol=2
    ...
    wwa_text = "### 🔔 Active Weather Alerts in Texas:\n"
    if alerts:
        event_groups = {}
        for alert in alerts:
            evt = alert['event']
        ...
        wwa_text = "✅ No active weather alerts for Texas at this time."
...
# --- *** NEW DUAL-SAVE LOGIC *** ---
    ...
    dynamic_filename = f"tx_detailed_weather_{timestamp_str}.png"
    static_filename = "tx_detailed_weather.png" # For the GUI preview
    ...
    # plt.show() # <-- REMOVED to prevent pop-up
    plt.close(fig)
...
if __name__ == "__main__":
    ...
    """
    try:
        generate_texas_weather_map() # <-- CHANGED: Synchronous call

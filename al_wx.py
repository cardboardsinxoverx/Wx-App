...
# -*- coding: utf-8 -*-
"""
Standalone Alabama (AL) Weather Map Generator.
...
NEXRAD radar image to generate a detailed weather map for the state of Alabama.

The final map is saved as 'al_detailed_weather.png'.
"""
...
# --- Configuration (ALABAMA) ---
MAP_EXTENT = [-88.6, -84.7, 30.0, 35.2] # [lon_min, lon_max, lat_min, lat_max]
CITIES = {
    "Birmingham": (33.5639, -86.7536, "KBHM"),
    "Montgomery": (32.3011, -86.3939, "KMGM"),
    "Mobile": (30.6869, -88.2428, "KMOB"),
    "Huntsville": (34.6433, -86.7750, "KHSV"),
    "Tuscaloosa": (33.2206, -87.6111, "KTCL"),
    "Dothan": (31.3214, -85.4494, "KDHN"),
    "Auburn": (32.6591, -85.4403, "KAUO"),
    "Anniston": (33.5878, -85.8569, "KANB"),
}
CITY_OFFSETS = {
    "Birmingham": (0.1, 0.1),
    "Montgomery": (0.1, -0.1),
...
SILENT_STATIONS = {
    # In-state fill-ins
    "Evergreen": (31.424, -86.953, "KGZH"),
    "Selma": (32.346, -87.058, "KSEM"),
    "Demopolis": (32.482, -87.896, "KDYA"),
    
    # Border stations for "pinning" the edges
    "Meridian (MS)": (32.3325, -88.7517, "KMEI"),
    "Columbus (GA)": (32.5161, -84.9389, "KCSG"),
    "Chattanooga (TN)": (35.0353, -85.2038, "KCHA"),
    "Pensacola (FL)": (30.4733, -87.1867, "KPNS"),
}
# ---
FIG_BG_COLOR = '#b0c4de'
LAND_COLOR = '#f5f5dc'
...
def add_satellite_overlay(fig, ax, clip_patch, text_outline, bt, x_coords, y_coords, sat_proj):
    ...
    try:
        ...
        # This code correctly places the colorbar *below* the map axes.
...
def get_nws_alerts(state='AL'):
    """Fetches active NWS alerts for a given state (e.g., 'AL')."""
...
def generate_alabama_weather_map(): # <-- No longer async
    """Generate and save the Alabama weather map."""
    
    logging.info("Generating Alabama weather map...")
    
    ALL_STATIONS_AL = CITIES.copy()
    ALL_STATIONS_AL.update(SILENT_STATIONS)
    ...
    metar_cache_file = "metar_al_cache.json"
    ...
    if metar_results is None:
        ...
        metar_results = asyncio.run(fetch_all_metars_async(ALL_STATIONS_AL))
    ...
    alert_cache_file = "alerts_al_cache.json"
    alerts = load_json_cache(alert_cache_file, CACHE_DURATION_SECONDS)
    
    if alerts is None:
        logging.info("Fetching NWS alerts for Alabama...")
        alerts = get_nws_alerts('AL') 
    ...
    for city, temp in all_temperatures.items():
        lat, lon, _ = ALL_STATIONS_AL[city]
        ...
        if temp is not None:
            ...
            if wind_dir is not None and wind_spd is not None and wind_spd > 0:
                # Convert from meteorological (from) to cartesian (to)
    ...
    if len(temp_values) >= 4:
        ...
        try:
            ...
            # Use cubic interpolation for smooth contours
            ...
            # Fill in NaNs at the edges with nearest neighbor
    ...
    # White outline for text to make it readable
    text_outline = [pe.withStroke(linewidth=3, foreground='white')]
    
    current_time = datetime.now().strftime('%Y-%m-%d %I:%M %p') 
    fig.suptitle(f"Alabama - Detailed Weather Map\nGenerated: {current_time}", 
    ...
    clip_patch = None
    try:
        logging.info("Creating state clipping path for Alabama...")
        ...
        if not state_geometries:
            raise ValueError("Could not find Alabama in shpreader.")
        
        # Combine geometries (in case of islands, etc.)
        combined_geom = unary_union(state_geometries)
        if not combined_geom:
            raise ValueError("Unary union of Alabama geometry failed.")
            
        # Convert the Shapely geometry to a Matplotlib Path
        paths = geos_to_path(combined_geom)
        if not paths:
            raise ValueError("geos_to_path returned no paths for Alabama.")
        ...
        if clip_patch:
            ...
            raise ValueError("PathPatch creation failed for Alabama.")
    except Exception as e:
        logging.error(f"Failed to create clip patch for Alabama. Error: {e}")
        clip_patch = None
    ...
    for city, temp in all_temperatures.items():
        base_lat, base_lon, icao = ALL_STATIONS_AL[city]
    ...
    # --- Colorbar and Legend ---
    # This code correctly places the main temp colorbar *outside* the map axes.
    divider = make_axes_locatable(ax)
    ...
    # Changed loc to 'lower right' and bbox_to_anchor to (1.0, 0.0)
    # This places the legend outside the map, anchored at its bottom-right corner.
    ...
    wwa_text = "### 🔔 Active Weather Alerts in Alabama:\n" 
    if alerts:
        event_groups = {}
        for alert in alerts:
            evt = alert['event']
        ...
        wwa_text = "✅ No active weather alerts for Alabama at this time."
...
# --- *** NEW DUAL-SAVE LOGIC *** ---
    ...
    dynamic_filename = f"al_detailed_weather_{timestamp_str}.png"
    static_filename = "al_detailed_weather.png" # For the GUI preview
...
if __name__ == "__main__":
    ...
    try:
        generate_alabama_weather_map() # <-- Now a synchronous call

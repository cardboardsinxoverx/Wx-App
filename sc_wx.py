...
# -*- coding: utf-8 -*-
"""
Standalone South Carolina (SC) Weather Map Generator.
...
NEXRAD radar image to generate a detailed weather map for the state of South Carolina.

The final map is saved as 'sc_detailed_weather.png'.
"""
...
# --- Configuration (SOUTH CAROLINA) ---
MAP_EXTENT = [-83.5, -78.4, 31.9, 35.3] # [lon_min, lon_max, lat_min, lat_max]
CITIES = {
    "Columbia": (33.9388, -81.1194, "KCAE"),
    "Charleston": (32.8986, -80.0405, "KCHS"),
    "Greenville-Spartanburg": (34.8956, -82.2186, "KGSP"),
    "Myrtle Beach": (33.6797, -78.9283, "KMYR"),
    "Florence": (34.1856, -79.7239, "KFLO"),
    "Hilton Head": (32.2244, -80.6975, "KHXD"),
    "Rock Hill": (34.9878, -81.0572, "KUZA"),
    "Aiken": (33.6475, -81.6858, "KAIK"),
    "Anderson": (34.5100, -82.7100, "KAND"),
}
CITY_OFFSETS = {
    "Columbia": (0.1, 0.1),
    "Charleston": (0.1, 0.1),
    "Myrtle Beach": (0.1, -0.1),
    "Greenville-Spartanburg": (0.1, 0.1),
}
SILENT_STATIONS = {
    # In-state
    "Greenwood": (34.25, -82.15, "KGRD"),
    "Orangeburg": (33.47, -80.86, "KOGB"),
    "Sumter": (33.995, -80.3613, "KSMS"),
    "Walterboro": (32.92, -80.64, "KRBW"),
    # Border stations for "pinning" the edges
    "Charlotte (NC)": (35.22, -80.95, "KCLT"),
    "Asheville (NC)": (35.43, -82.54, "KAVL"),
    "Wilmington (NC)": (34.2711, -77.9029, "KILM"),
    "Augusta (GA)": (33.37, -81.96, "KAGS"),
    "Savannah (GA)": (32.13, -81.20, "KSAV"),
}
# ---
FIG_BG_COLOR = '#b0c4de'
LAND_COLOR = '#f5f5dc'
...
# --- Helper Functions (Weather/METAR Parsing) ---
...
def get_nws_alerts(state='SC'):
    """Fetches active NWS alerts for a given state (e.g., 'SC')."""
...
def generate_south_carolina_weather_map(): # <-- [CHANGED]
    """Generate and save the South Carolina weather map."""
    
    logging.info("Generating South Carolina weather map...")
    
    ALL_STATIONS_SC = CITIES.copy()
    ALL_STATIONS_SC.update(SILENT_STATIONS)
    ...
    metar_cache_file = "metar_sc_cache.json"
    ...
    if metar_results is None:
        ...
        metar_results = asyncio.run(fetch_all_metars_async(ALL_STATIONS_SC))
    ...
    for city, data in metar_results.items():
        if data:
            all_temperatures[city] = data.get("temp_f")
            all_dew_points[city] = data.get("dew_point_f")
            all_wind_directions[city] = data.get("wind_dir")
            all_wind_speeds[city] = data.get("wind_spd")
            all_altimeters[city] = data.get("alt_inHg")
            all_weather_conditions[city] = data.get("condition")
        else:
    ...
    alert_cache_file = "alerts_sc_cache.json"
    alerts = load_json_cache(alert_cache_file, CACHE_DURATION_SECONDS)
    
    if alerts is None:
        logging.info("Fetching NWS alerts for South Carolina...")
        alerts = get_nws_alerts('SC')
    ...
    for city, temp in all_temperatures.items():
        lat, lon, _ = ALL_STATIONS_SC[city]
    ...
    fig.suptitle(f"South Carolina - Detailed Weather Map\nGenerated: {current_time}",
                 fontsize=FONT_SIZE_TITLE, fontweight="bold", y=0.80, color='navy',
                 path_effects=text_outline) 
    ...
    clip_patch = None
    try:
        logging.info("Creating state clipping path for South Carolina...")
        target_states = ['South Carolina']
        state_geometries = []
        # --- Use 50m for speed ---
        ...
        if not state_geometries:
            raise ValueError("Could not find South Carolina in shpreader.")
        combined_geom = unary_union(state_geometries)
        if not combined_geom:
            raise ValueError("Unary union of South Carolina geometry failed.")
        paths = geos_to_path(combined_geom)
        if not paths:
            raise ValueError("geos_to_path returned no paths for South Carolina.")
        ...
        if clip_patch:
            ...
            raise ValueError("PathPatch creation failed for South Carolina.")
    except Exception as e:
        logging.error(f"Failed to create clip patch for South Carolina. Error: {e}")
        clip_patch = None
    if not clip_patch:
    ...
    for city, temp in all_temperatures.items():
        base_lat, base_lon, icao = ALL_STATIONS_SC[city]
        ...
        else: # This is for the 'silent' stations
            if temp is not None:
                color = cmap(norm(temp))
    ...
    # --- *** MODIFIED: Legend placed in bottom-left *** ---
    ...
    wwa_text = "### 🔔 Active Weather Alerts in South Carolina:\n"
    if alerts:
        event_groups = {}
        for alert in alerts:
            evt = alert['event']
        ...
        wwa_text = "✅ No active weather alerts for South Carolina at this time."
 ...
 # --- *** NEW DUAL-SAVE LOGIC *** ---
    ...
    dynamic_filename = f"sc_detailed_weather_{timestamp_str}.png"
    static_filename = "sc_detailed_weather.png" # For the GUI preview
...
if __name__ == "__main__":
    ...
    """
    try:
        generate_south_carolina_weather_map() # <-- [CHANGED]
